...
export async function d20Roll({
  ...
  mergeInputArgumentsIntoRollConfig(
    rollConfig,
    parts,
    event,
    rollMode,
    ...
    new Dialog(
      {
        ...
        buttons: {
          normal: {
            ...
            callback: (html) =>
              (roll = performD20RollAndCreateMessage(
...
export function getAttackRollBonus({
  parts = [],
  partsExpressionReplacements = [],
  data = {},
  options = {},
...
async function performD20RollAndCreateMessage(
  form,
  {
    parts,
    partsExpressionReplacements,
  ...
  if (!parts.includes("@tool") && !parts.includes("@ritual")) {
    ...
    if (!form || !form.d20?.value || form.d20?.value == 1) {
      // type coercion is expected here!  It's a string value
      parts.unshift(`1d20`);
  ...
  if (isAttackRoll && form !== null) {
    ...
    Object.keys(CONFIG.DND4E.commonAttackBonuses).forEach(
      function (key, index) {
    ...
    for (let targetIndex = 0; targetIndex < numberOfTargets; targetIndex++) {
      ...
      for (let [k, v] of Object.entries(form)) {
        if (v.checked) {
          ...
          if (
            (individualAttack && parseInt(tabIndex) === targetIndex) || // check if Individual Attack Bonuses
            !individualAttack
          ) {
            //otherwise just use Universal Attack Bonuses
      ...
      if (game.settings.get("dnd4e", "collapseSituationalBonus")) {
        let total = 0;
        targetBonuses.forEach(
          (bonus) =>
            (total +=
              CONFIG.DND4E.commonAttackBonuses[bonus.substring(1)].value),
        );
  ...
  for (
    let rollExpressionIdx = 0;
    rollExpressionIdx < allRollsParts.length;
    rollExpressionIdx++
  ) {
    const rollExpression = allRollsParts[rollExpressionIdx];
    let subroll;
    try {
      subroll = await roll.addNewRoll(
        rollExpression,
        partsExpressionReplacements,
        data,
        options,
      ...
      ui.notifications.error(
        "Error trying to roll: " + rollExpression.join("+") + ":" + err,
      );
      throw err;
    }
    ...
    if (isAttackRoll && targets.length > rollExpressionIdx) {
      ...
      let targDefVal =
        targets[rollExpressionIdx].document.actor.system.defences[
          options.attackedDef
        ]?.value;
...
export async function damageRoll({
  ...
  const doRoll = (html, hitType) => {
    ...
    return performDamageRollAndCreateChatMessage(
      html[0].querySelector("form"),
      rollConfig,
    );
  };
    ...
    dialogConfig.buttons = {
      ...
      normal: {
        label: game.i18n.localize(
          allowCritical ? "DND4E.Normal" : "DND4E.Roll",
        ),
...
async function performDamageRollAndCreateChatMessage(
  form,
  {
    parts,
    partsCrit,
  ...
  if (data.bonus) {
    //stopgap fix because bonus damage type is not being recorded properly
    if (parts[parts.length - 1] === "@bonus") {
      let index = data.bonus.lastIndexOf("[");
      if (index >= 0) {
        parts[parts.length - 1] =
          "(" + data.bonus.slice(0, index) + ")" + data.bonus.slice(index);
      } else {
  ...
  if (hitType === "normal") {
    ...
    roll = RollWithOriginalExpression.createRoll(
      parts,
      partsExpressionReplacement,
      data,
      options,
...
function mergeInputArgumentsIntoRollConfig(
  rollConfig,
  parts,
  event,
  rollMode,
