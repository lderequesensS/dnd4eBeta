...
export class RollWithOriginalExpression extends Roll {
  ...
  constructor(formula, data = {}, options = {}) {
    super(
      ...
      foundry.utils.mergeObject(
        { expression: formula, originalFormula: formula },
        options,
      ),
    );
  ...
  static createRoll(
    ...
    if (game.settings.get("dnd4e", "showRollExpression")) {
      ...
      tempExpression = tempExpression.map((part) => {
        ...
        if (
          trimmedPart.indexOf("(") === 0 &&
          trimmedPart.indexOf("]") === trimmedPart.length - 1
        ) {
    ...
    options.parts = parts;
    options.expressionArr = this._createExpression(
      parts,
      expressionPartsReplacements,
    );
  ...
  static CHAT_TEMPLATE =
    "systems/dnd4e/templates/chat/roll-template-single.html";
      ...
      for (let i = 0; i < formula.length; i++) {
        ...
        if (char === ")") {
          ...
          if (openBracket === 0) {
            ...
            const formData = this.replaceInnerVariables(
              workingFormula,
              expressionParts[expressionIdx],
              spanId,
            );
            ...
            formData.formula = this.includeOuterBracketsIfFormulaIsFlavoured(
              formula,
              i,
              formData.formula,
            );
   ...
   */
  includeOuterBracketsIfFormulaIsFlavoured(
    fullFormula,
    currentIndex,
    currentWorkingFormula,
  ) {
    if (
      currentIndex < fullFormula.length &&
      fullFormula.charAt(currentIndex + 1) === "["
    ) {
      ...
      try {
        ...
        if (!(vars.length === 1 && vars[0] === expression)) {
          for (let innerIndex = 0; innerIndex < vars.length; innerIndex++) {
            ...
            let replacementStr =
              this.options.formulaInnerData[variable.substring(1)];
            if (!replacementStr) {
              // may be a complex replacement: e.g. details.level
              replacementStr = Helper.replaceData(
                variable,
                this.options.formulaInnerData,
              );
              if (!replacementStr) {
            ...
            activeExpression = activeExpression.replace(
              variable,
              `<span id="exp${spanId}" onmouseenter="mouseEnter('${spanId}')" onmouseleave="mouseLeave('${spanId}')">${variable}</span>`,
            );
            ...
            activeFormula = activeFormula.substring(
              indexOfReplacement + ("" + replacementStr).length,
            );
          }
          // get the rest
