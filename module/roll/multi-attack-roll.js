...
export class MultiAttackRoll extends Roll {
  ...
  static CHAT_TEMPLATE =
    "systems/dnd4e/templates/chat/roll-template-multiattack.html";
  ...
  async addNewRoll(parts, expressionPartsReplacements, data, options) {
    const roll = await RollWithOriginalExpression.createRoll(
      parts,
  ...
  populateMultirollData(targDataArray, critStateArray) {
    for (let [i, r] of this.rollArray.entries()) {
      ...
      if (
        game.settings.get("dnd4e", "automationCombat") &&
        targDefVal !== undefined
      ) {
  ...
  async render(chatOptions = {}) {
    ...
    for (let roll of this._multirollData) {
      ...
      roll.tooltip = await renderTemplate(this.constructor.TOOLTIP_TEMPLATE, {
        parts,
      });
    }
