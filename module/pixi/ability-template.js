...
export class MeasuredTemplate4e extends MeasuredTemplate {
  static getDistanceCalc(item) {
    ...
    try {
      ...
      area = Roll.replaceFormulaData(area, item.actor.getRollData(), {
        missing: 0,
        warn: true,
      });
      let r = new Roll(area);
  ...
  static fromItem(item) {
    ...
    if (
      item.system.rangeType === "closeBlast" ||
      item.system.rangeType === "rangeBlast"
    ) {
  ...
  static _computeShape(wrapper) {
    if (
      this.document.flags.dnd4e?.templateType === "burst" ||
      (this.document.t === "circle" &&
        ui.controls.activeControl === "measure" &&
        ui.controls.activeTool === "burst" &&
        !this.document.flags.dnd4e?.templateType)
    ) {
      const { t, distance, direction, angle, width } = this.document;
      return new PIXI.Polygon(canvas.grid.getCircle({ x: 0, y: 0 }, distance));
    } else if (
      this.document.flags.dnd4e?.templateType === "blast" ||
      (this.document.t === "circle" &&
        ui.controls.activeControl === "measure" &&
        ui.controls.activeTool === "blast" &&
        !this.document.flags.dnd4e?.templateType)
    ) {
      const { t, distance, direction, angle, width } = this.document;

      return new PIXI.Polygon(
        canvas.grid.getCone({ x: 0, y: 0 }, distance, direction, angle),
      );
    }
    ...
    if (
      ...
      if (this.document.flags.dnd4e?.closeBurst) {
        switch (this.document.flags.dnd4e?.closeBurst) {
          case "lg":
            d = Math.max(
              Math.round((this.document.distance - 1.0) * 10) / 10,
              0,
            );
          ...
          case "huge":
            d = Math.max(
              Math.round((this.document.distance - 1.5) * 10) / 10,
              0,
            );
            text = `${game.i18n.localize("DND4E.rangeCloseBurst")} ${d} \n(${DND4E.actorSizes[this.document.flags.dnd4e.closeBurst].label})`;
            break;
          case "grg":
            d = Math.max(
              Math.round((this.document.distance - 2.0) * 10) / 10,
              0,
            );
            text = `${game.i18n.localize("DND4E.rangeCloseBurst")} ${d} \n(${DND4E.actorSizes[this.document.flags.dnd4e.closeBurst].label})`;
            break;
          default:
            d = Math.max(
              Math.round((this.document.distance - 0.5) * 10) / 10,
              0,
            );
            text = `${game.i18n.localize("DND4E.rangeCloseBurst")} ${d}`;
        }
  ...
  static _refreshShape(wrapper) {
    ...
    this.ray = new Ray(
      { x, y },
      canvas.grid.getTranslatedPoint({ x, y }, direction, distance),
    );
...
export class TemplateLayer4e extends TemplateLayer {
  ...
  static _onDragLeftMove(wrapper, event) {
    ...
    preview.document.direction = Math.normalizeDegrees(
