...
TokenBarHooks.generatePowerGroups = (actor) =>
  actor.sheet._generatePowerGroups();

TokenBarHooks.updatePowerAvailable = (actor, power) =>
...
TokenBarHooks.quickSave = (actor, event) =>
  new SaveThrowDialog(actor)._updateObject(event, { save: 0, dc: 10 });

TokenBarHooks.saveDialog = (actor, event) => actor.sheet._onSavingThrow(event);

TokenBarHooks.healDialog = (actor, event) =>
  actor.sheet._onHealMenuDialog(event);

TokenBarHooks.rechargePower = (actor, power, event) =>
...
TokenBarHooks.rollPower = (actor, power, event) => {
  ...
  return actor.usePower(power, {
    configureDialog: !fastForward,
    fastForward: fastForward,
  });
};

TokenBarHooks.rollSkill = (actor, checkId, event) =>
  actor.rollSkill(checkId, { event: event });

TokenBarHooks.rollAbility = (actor, checkId, event) =>
...
TokenBarHooks.actionPoint = (actor, event) =>
  actor.sheet._onActionPointDialog(event);
...
TokenBarHooks.generateItemTooltip = async (actor, item) =>
  (await actor._sheet._generateItemSummary(item)).get(0).outerHTML;
