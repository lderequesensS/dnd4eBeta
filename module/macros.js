...
export function rollItemMacro(itemName, type = "DOCUMENT.Item") {
  ...
  if (!actor)
    return ui.notifications.warn(game.i18n.localize("DND4E.ControlledNoActor"));

  // Get matching items
  const items = actor ? actor.items.filter((i) => i.name === itemName) : [];
  if (items.length > 1) {
    ui.notifications.warn(
      game.i18n.format("MACRO.4eMultipleTargetsWarn", {
        actor: actor.name,
    ...
    return ui.notifications.warn(
      game.i18n.format("MACRO.4eMissingTargetWarn", {
        actor: actor.name,
        type: game.i18n.localize(type),
        name: itemName,
      }),
