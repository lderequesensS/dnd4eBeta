...
export const addChatMessageContextOptions = function (html, options) {
  let isAttackRoll = (li) => {
    ...
    return (
      ...
      li[0].querySelector(".hit-prediction")
    );
  };
  ...
  let canApplyEffect = (li, type) => {
    ...
    const tokenUUID = li[0]
      .querySelector("[data-token-id]")
      ?.dataset.tokenId.split(".");

    const actor = tokenUUID
      ? game.scenes.get(tokenUUID[1])?.tokens.get(tokenUUID[3])?.actor
  ...
  for (const [effectType, l] of Object.entries(
    game.dnd4e.config.powerEffectTypes,
  )) {
    options.push({
...
export function chatMessageListener(html) {
  html.on(
    "click",
    ".chat-damage-button",
    this.clickRollMessageDamageButtons.bind(this),
  );

  html.on("click", ".target", this.clickTokenActorName.bind(this));
  html
    .on("mouseenter", ".target", this.hoverTokenActorName.bind(this))
    .on("mouseleave", ".target", this.hoverTokenActorName.bind(this));
...
export const clickTokenActorName = function (event) {
  ...
  const tokenID =
    event.currentTarget.getAttribute("data-target-id") ||
    event.currentTarget.getAttribute("target-id"); //second one was for legasy where improper typing is used, will get rid of in a month or so
  if (!tokenID) return;

  const token = canvas.tokens.get(tokenID);

  if (!token)
    return console.log(`Token ID: ${tokenID} does not exist in this scene.`);
...
function selectTargetTokens(li, targetType) {
  ...
  if (targetType === "hit") {
    ...
    for (const roll of message.rolls) {
      if (
        [
          game.i18n.localize("DND4E.AttackRollHit"),
          game.i18n.localize("DND4E.AttackRollHitCrit"),
        ].includes(roll.options.multirollData.hitstate)
      ) {
        canvas.tokens
          .get(roll.options.multirollData.targetID)
          .control({ releaseOthers: false });
      }
    }
    ...
    for (const roll of message.rolls) {
      if (
        [
          game.i18n.localize("DND4E.AttackRollMiss"),
          game.i18n.localize("DND4E.AttackRollMissCrit"),
        ].includes(roll.options.multirollData.hitstate)
      ) {
        canvas.tokens
          .get(roll.options.multirollData.targetID)
  ...
  if (multiplier < 0 || trueDamage) {
    return Promise.all(
      canvas.tokens.controlled.map((t) => {
        ...
        console.log(
          multiplier < 0
            ? `Amount Healed for: ${roll.total}`
            : `True Damage Dealt: ${roll.total}`,
        );
  ...
  if (rollTotalRemain) {
    ...
    if (length === 0) {
      ...
      for (let i = 0; i < length; i++) {
        damageDealt[i][0] +=
          0 | (rollTotalRemain / length + (i < rollTotalRemain % length));
      }
    }
  }
...
export function _onDiceRollClick(wrapper, event) {
  ...
  if (
    ...
    (event.target.tagName.toLowerCase() === "b" &&
      event.target.parentElement.classList.contains("target"))
  ) {
    return;
  }
...
export async function _processDiceCommand(wrapper, ...args) {
  ...
  for (const match of matches) {
    ...
    const roll = Roll.create(
      actor ? game.helper.commonReplace(formula, actor) : formula,
      rollData,
    );
