...
import {
  ...
  DifficultTerrainShader4e,
  Region4e,
} from "./regionBehavoirs/difficult-terrain.js";

import {
  Helper,
  handleApplyEffectToToken,
  handleDeleteEffectToToken,
  handlePromptEoTSaves,
  handleAutoDoTs,
  performPreLocalization,
} from "./helper.js";
...
Hooks.once("init", async function () {
  console.log(
    `D&D4e | Initializing Dungeons & Dragons 4th Edition System\n${DND4E.ASCII}`,
  );
  ...
  CONFIG.RegionBehavior.dataModels.difficultTerrain =
    DifficultTerrainRegionBehaviorType;
  HighlightRegionShader = DifficultTerrainShader4e;

  CONFIG.RegionBehavior.typeLabels.difficultTerrain =
    "DND4E.difficultTerrain.Label"; //"DND4E.difficultTerrain.Label";
  ...
  game.dnd4e.quickSave = (actor) =>
    game.dnd4e.tokenBarHooks.quickSave(actor, null);
...
Hooks.once("ready", function () {
  ...
  game.socket.on("system.dnd4e", (data) => {
    ...
    else if (data.operation === "deleteTokenEffect")
      handleDeleteEffectToToken(data);
  ...
  const cv =
    game.settings.get("dnd4e", "systemMigrationVersion") ||
    game.world.flags.dnd4e?.version;
  ...
  if (cv && foundry.utils.isNewerVersion(cmv, cv)) {
    ui.notifications.error(
      game.i18n.localize("MIGRATION.4eVersionTooOldWarning"),
      { permanent: true },
    );
  }
...
Hooks.on("renderChatMessage", (app, html, data) => {
  ...
  if (game.settings.get("dnd4e", "autoCollapseItemCards"))
...
function libWrapperInit() {
  ...
  libWrapper.register("dnd4e", "Region.prototype._draw", Region4e._draw);

  libWrapper.register("dnd4e", "Combat.prototype.nextTurn", Turns._onNextTurn);
...
Hooks.on("renderChatMessage", (message, html, data) => {
  if (
    message.flags.core?.initiativeRoll === true ||
    message.flags?.dnd4e?.roll?.type == "init"
  ) {
    if (html) {
      const insertPart = Helper.initTooltip(message.content);
      html[0].innerHTML = html[0].innerHTML.replace(
        /(<h4 class=\"dice-total\">)[0-9|.]+(<\/h4>)/g,
        `$1${insertPart}$2`,
      );
    }
  }
...
Hooks.on("renderCombatTracker", (app, html, context) => {
  ...
  html.find(".token-initiative").each((i, el) => {
    let combatant = app.viewed.combatants.get(
      el.parentElement.dataset.combatantId,
    );
