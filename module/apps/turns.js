...
export class Turns {
  static async _onNextTurn(wrapped) {
    ...
    const nextTurn =
      game.combat.turn + 1 < game.combat.turns.length
        ? game.combat.turn + 1
        : 0;
    const nextInit =
      game.combat.turn + 1 < game.combat.turns.length
        ? game.combat.turns[game.combat.turn + 1].initiative
        : game.combat.turns[0].initiative;
    ...
    for (let t of game.combat.turns) {
      ...
      for (let e of t.token.actor.effects) {
        ...
        if (!durationType) {
          ...
          if (
            (currentInit <= effectData.durationTurnInit &&
              currentRound >= e.duration.rounds &&
              t.id === game.combat.combatant.id) ||
            (currentRound > e.duration.rounds &&
              t.id === game.combat.combatant.id)
          ) {
            toDelete.push(e.id);
          ...
          durationType === "endOfUserTurn" ||
          durationType === "endOfUserCurrent"
        ) {
          if (
            currentInit <= effectData.durationTurnInit &&
            currentRound >= e.duration.rounds
          ...
          durationType === "startOfTargetTurn" ||
          durationType === "startOfUserTurn"
        ) {
          if (
            (nextInit <= effectData.durationTurnInit &&
              currentRound == e.duration.rounds) ||
            currentRound > e.duration.rounds ||
            (nextTurn <= currentTurn &&
              nextInit <= effectData.durationTurnInit &&
              currentRound + 1 == e.duration.rounds)
          ) {
            toDelete.push(e.id);
          }
        }
        ...
        if (currentTurn === game.combat.combatants.size) {
          if (durationType === "endOfUserTurn") {
            if (
              effectData.durationTurnInit < currentInit &&
              e.duration.rounds <= currentRound
            ) {
              toDelete.push(e.id);
            }
          }
