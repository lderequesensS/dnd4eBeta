...
export class AttributeBonusDialog extends DocumentSheet4e {
  ...
  async _updateObject(event, formData) {
    ...
    if (this.options?.skill) {
      updateData[`${this.options.target}.armourCheck`] =
        formData["system.armourCheck"];
      ...
      updateData[`${this.options.target}.altability`] =
        formData["system.altability"];
  ...
  _onBonusAdd(event) {
    ...
    return this.object.update({
      [`${this.options.target}.bonus`]: bonusData.concat(newBonus),
    });
  }
  ...
  _onBonusDelete(event) {
    ...
    const bonus = duplicate(
      Helper.byString(this.options.target, this.object).bonus,
    );
