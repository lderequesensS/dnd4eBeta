...
export default class CustomSkillConfig extends FormApplication {
  ...
  async _updateObject(event, formData) {
    ...
    if (!Array.isArray(formData[Object.keys(formData)[0]])) {
      formData = [formData].filter((newObj) => {
        if (coreData[newObj.id]) {
          console.log(
            `Aborted Custom Skill creation of <strong>"${newObj.label}"</strong>, for matching id of <strong>"${newObj.id}"</strong> with core skill <strong>"${game.dnd4e.config.skills[newObj.id]}"</strong>.`,
          );
    ...
    // let newSKills
    let transposedArray = Object.keys(formData[Object.keys(formData)[0]]).map(
      (index) => {
        const newObj = {};
  ...
  updateActorsSkills(newDataArray) {
    ...
    if (oldData.length && newDataArray.length) {
      newDataArray.forEach((newObj) => {
        const foundInOld = oldData.some((oldObj) =>
          areObjectsEqual(newObj, oldObj),
        );
        if (!foundInOld) {
    ...
    if (oldData.length) {
      oldData.forEach((oldObj) => {
        const foundInNew = newDataArray.some((newObj) =>
          areObjectsEqual(oldObj, newObj),
        );
        if (!foundInNew) {
  ...
  async addCustomSkill(skillData) {
    ...
    for (const actor of game.actors) {
      ...
      updateData[`system.skills.${skillData.id}.armourCheck`] =
        skillData.armourCheck;
