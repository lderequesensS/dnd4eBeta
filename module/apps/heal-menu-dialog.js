...
export class HealMenuDialog extends DocumentSheet4e {
  ...
  getData(options) {
    const hpMax =
      this.object.system.attributes.hp.value ===
  ...
  async _updateObject(event, formData) {
    ...
    let roll = await Helper.rollWithErrorHandling(formData.bonus, {
    ...
    if (healType === "tempHP") {
      ...
      this.object.system.details.surges.value == 0 &&
      this.object.system.attributes.hp.value <= 0
    ) {
    ...
    if (formData["spend-healing-surge"] === true) {
      ...
      updateData[`system.details.surges.value`] = Math.max(
        this.object.system.details.surges.value - 1,
        0,
      );
    ...
    const rollMessage =
      formData.bonus && formData.bonus !== ""
        ? ` + ${roll.total} (${roll.formula} => ${roll.result})`
        : "";
