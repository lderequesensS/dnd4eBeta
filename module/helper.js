export class Helper {
  ...
  static getWeaponUse(itemData, actor) {
    if (
      itemData.weaponUse === "none" ||
      (itemData.weaponType === "none" && actor.itemTypes.weapon.length === 0)
    )
      return null;
    let weaponUse = itemData.weaponUse
      ? actor.items.get(itemData.weaponUse)
      : null;
    //If default weapon is in use, find a sutable weapon
    if (
      itemData.weaponUse === "default" ||
      itemData.weaponUse === "defaultOH"
    ) {
      let setMelee = [
        "melee",
        "simpleM",
      ...
      return actor.itemTypes.weapon
        .sort((i) => (i.system.weaponHand === "hOff" ? 1 : -1))
        .find((i) => {
          // Flush off-hand to the end
          if (i.system.equipped) {
            ...
            if (itemData.weaponType === "meleeRanged") {
              if (
                setMelee.includes(i.system.weaponType) ||
                setRanged.includes(i.system.weaponType)
              )
                if (
                  itemData.weaponUse === "defaultOH" &&
                  i.system.weaponHand === "hOff"
                )
                  return i;
                else if (itemData.weaponUse === "default") return i;
              ...
              if (
                setRanged.includes(i.system.weaponType) ||
                i.system.properties.tlg ||
                i.system.properties.thv
              )
                if (
                  itemData.weaponUse === "defaultOH" &&
                  i.system.weaponHand === "hOff"
              ...
              if (
                i.system.properties.imp ||
                i.system.properties.impA ||
                i.system.properties.impD
              )
                if (
                  itemData.weaponUse === "defaultOH" &&
                  i.system.weaponHand === "hOff"
  ...
  static lacksRequiredWeaponEquipped(itemData, weaponUse) {
    ...
    const powerNeedsAWeapon =
      itemData.weaponType &&
      !["none", "implement", "any"].includes(itemData.weaponType) &&
      itemData.weaponUse !== "none";
  ...
  static async applyEffects(
    ...
    if (actorData.effects) {
      ...
      if (debug) {
        console.log(
          `${debug} Debugging ${effectType} effects for ${powerData.name}.	Supplied Weapon: ${weaponData?.name}`,
        );
      }
      ...
      if (game.settings.get("dnd4e", "inhEnh")) {
        ...
        enhValue = Math.max(
          weaponInnerData?.enhance || 0,
          Helper.findKeyScale(
            actorData.system.details.level,
      ...
      const effects = actorData
        .getActiveEffects()
        .filter((effect) => effect.disabled === false);
      effects.forEach((effect) => {
        effect.changes.forEach((change) => {
          if (
            change.key.startsWith(`power.${effectType}`) ||
            (weaponInnerData && change.key.startsWith(`weapon.${effectType}`))
          ) {
      ...
      if (globalMods[effectType].value != 0) {
        for (const [key, value] of Object.entries(globalMods[effectType])) {
          ...
          const adjValue =
            key == "untyped"
              ? value + globalMods[effectType].bonusValue
              : value;
          if (
            !["value", "bonus", "warn", "bonusValue", "label"].includes(key) &&
            adjValue != 0
          ) {
            effectsToProcess.push({
      ...
      if (effectsToProcess.length > 0) {
        if (debug) {
          ...
          effectsToProcess.forEach((effect) =>
            console.log(
              `${debug} ${effect.name} : ${effect.key} = ${effect.value}`,
            ),
          );
        }
        ...
        if (powerInnerData.weaponType) {
          ...
          switch (powerInnerData.weaponType) {
            ...
            case "implement":
              if (weaponInnerData) {
                if (weaponInnerData.proficientI)
                  suitableKeywords.push("proficient");
              }
              break;
            case "any":
              if (weaponInnerData) {
        ...
        if (weaponInnerData) {
          if (!weaponInnerData.properties.two) {
            //Skip if it's tagged two-handed
        ...
        if (powerInnerData.attack?.isBasic) {
          ...
          if (suitableKeywords.includes("melee"))
            suitableKeywords.push("mBasic");
          if (suitableKeywords.includes("ranged"))
            suitableKeywords.push("rBasic");
        }

        if (debug) {
          console.log(
            `${debug} based on power source, effect type, damage type and (if weapon) weapon group, properties and damage type the following effect keys are suitable`,
          );
        ...
        if (debug) {
          console.log(
            `${debug} The following effects were deemed suitable by keyword filter`,
          );
        ...
        for (const effect of matchingEffects) {
          ...
          if (keyParts.length === 4) {
            ...
            const effectValueString = this.commonReplace(
              effect.value,
              actorData,
              powerInnerData,
              weaponInnerData,
            );
            const effectDice = await this.rollWithErrorHandling(
              effectValueString,
              { context: effect.key },
            );
            ...
            if (bonusType === "untyped") {
              if (newParts["untypedEffectBonus"]) {
                newParts["untypedEffectBonus"] =
                  newParts["untypedEffectBonus"] + effectValue;
                if (debug) {
                  console.log(
                    `${debug} ${effect.name} : ${effect.key} => ${effect.value} = ${effectValue}: Additional untyped Bonus.	They Stack.`,
                  );
                }
              } else {
                newParts["untypedEffectBonus"] = effectValue;
                if (debug) {
                  console.log(
                    `${debug} ${effect.name} : ${effect.key} => ${effect.value} = ${effectValue}: First untyped Bonus`,
                  );
                }
              }
            } else {
              ...
              if (newParts[key]) {
                if (newParts[key] < effectValue) {
                  ...
                  if (debug) {
                    console.log(
                      `${debug} ${effect.name} : ${effect.key} => ${effect.value} = ${effectValue}: Is greater than existing ${bonusType}, replacing`,
                    );
                  }
                } else {
                  if (debug) {
                    console.log(
                      `${debug} ${effect.name} : ${effect.key} => ${effect.value} = ${effectValue} : Is not greater than existing ${bonusType}, discarding`,
                    );
                  }
                }
              } else {
                ...
                if (debug) {
                  console.log(
                    `${debug} ${effect.name} : ${effect.key} => ${effect.value} = ${effectValue} : First ${bonusType} Bonus`,
                  );
                }
              }
            }
            ...
            ui.notifications.warn(
              `Tried to process a bonus effect that had too few/many .'s in it: ${effect.key}: ${effect.value}`,
            );
            console.log(
              `Tried to process a bonus effect that had too few/many .'s in it: ${effect.key}: ${effect.value}`,
            );
  ...
  static commonReplace(
    formula,
    actorData,
    powerInnerData,
    weaponInnerData = null,
    depth = 2,
    returnDataInsteadOfFormula = false,
  ) {
    if (depth < 0) return 0;
    ...
    if (returnDataInsteadOfFormula) {
      ...
      if (variables) {
        variables.forEach((variable) => {
          ...
          result[variable.substring(1)] = this.commonReplace(
            variable,
            actorData,
            powerInnerData,
            weaponInnerData,
      ...
      if (actorInnerData) {
        ...
        if (powerInnerData) {
          newFormula = newFormula.replaceAll(
            "@powerMod",
            !!actorInnerData.abilities[powerInnerData.attack?.ability]
              ? actorInnerData.abilities[powerInnerData.attack.ability].mod
              : "",
          );
        }
        ...
        newFormula = newFormula.replaceAll(
          "@strMod",
          actorInnerData.abilities["str"].mod,
        );
        newFormula = newFormula.replaceAll(
          "@conMod",
          actorInnerData.abilities["con"].mod,
        );
        newFormula = newFormula.replaceAll(
          "@dexMod",
          actorInnerData.abilities["dex"].mod,
        );
        newFormula = newFormula.replaceAll(
          "@intMod",
          actorInnerData.abilities["int"].mod,
        );
        newFormula = newFormula.replaceAll(
          "@wisMod",
          actorInnerData.abilities["wis"].mod,
        );
        newFormula = newFormula.replaceAll(
          "@chaMod",
          actorInnerData.abilities["cha"].mod,
        );

        newFormula = newFormula.replaceAll(
          "@lvhalf",
          Math.floor(actorInnerData.details.level / 2),
        );
        newFormula = newFormula.replaceAll("@lv", actorInnerData.details.level);
        newFormula = newFormula.replaceAll(
          "@tier",
          actorInnerData.details.tier,
        );

        newFormula = newFormula.replaceAll(
          "@atkMod",
          actorInnerData.modifiers.attack.value,
        );
        newFormula = newFormula.replaceAll(
          "@dmgMod",
          actorInnerData.modifiers.damage.value,
        );

        newFormula = newFormula.replaceAll(
          "@heroic",
          actorInnerData.details.level < 11 ? 1 : 0,
        );
        newFormula = newFormula.replaceAll(
          "@paragon",
          actorInnerData.details.level >= 11 &&
            actorInnerData.details.level < 21
            ? 1
            : 0,
        );
        newFormula = newFormula.replaceAll(
          "@epic",
          actorInnerData.details.level >= 21 ? 1 : 0,
        );
        ...
        newFormula = newFormula.replaceAll(
          "@bloodied",
          actorInnerData.details.isBloodied ? 1 : 0,
        );

        newFormula = newFormula.replaceAll(
          "@sneak",
          CONFIG.DND4E.SNEAKSCALE[actorInnerData.details.tier],
        );

        //targets @scale plus some #
        newFormula = newFormula.replace(/@scale(\d*)/g, (match, number) => {
          return this.findKeyScale(
            actorInnerData.details.level,
            CONFIG.DND4E.SCALE.basic,
            number - 1,
          );
        });
        ...
        console.log(
          "An actor data object without a .data property was passed to common replace. Probably passed actor.system by mistake!.	Replacing: " +
            formula,
        );
      }
    }
    ...
    newFormula = newFormula.replaceAll(
      "@powerLevel",
      powerInnerData?.level ? powerInnerData.level : 0,
    );
    let enhValue = weaponInnerData?.enhance || 0;
    ...
    if (weaponInnerData) {
      ...
      newFormula = newFormula.replaceAll(
        "@itemLevel",
        weaponInnerData.level ? weaponInnerData.level : 0,
      );
      ...
      if (powerInnerData.weaponType === "implement") {
        ...
        newFormula = newFormula.replaceAll(
          ...
          this.bracketed(
            this.commonReplace(
              weaponInnerData.critDamageFormImp,
              actorData,
              powerInnerData,
              weaponInnerData,
              depth - 1,
        ...
        newFormula = newFormula.replaceAll(
          ...
          this.bracketed(
            this.commonReplace(
              weaponInnerData.critDamageForm,
              actorData,
              powerInnerData,
              weaponInnerData,
              depth - 1,
      ...
      newFormula = newFormula.replaceAll(
        "@profBonusO",
        weaponInnerData.profBonus || 0,
      );
      newFormula = newFormula.replaceAll(
        "@profImpBonusO",
        weaponInnerData.profImpBonus || 0,
      );

      newFormula = newFormula.replaceAll(
        "@profImpBonus",
        weaponInnerData.proficientI ? weaponInnerData.profImpBonus || 0 : 0,
      );
      newFormula = newFormula.replaceAll(
        "@profBonus",
        weaponInnerData.proficient ? weaponInnerData.profBonus || 0 : 0,
      );
      ...
      if (newFormula.includes("@wepDice")) {
        ...
        let indexEnd =
          newFormula.substring(indexStart).indexOf(")") + 1 + indexStart;

        let weaponNum = newFormula
        ...
        for (let i = 0; i < parts.length; i++) {
          ...
          let quantity = this.commonReplace(
            parts[i][0],
            actorData,
            powerInnerData,
            weaponInnerData,
        ...
        const possibleDice = this.commonReplace(
          dice,
          actorData,
          powerInnerData,
          weaponInnerData,
        ...
        newFormula =
          newFormula.slice(0, indexStart) +
          newFormula.slice(indexEnd, newFormula.length);
      ...
      if (newFormula.includes("@powBase")) {
        let quantity = this.commonReplace(
          powerInnerData.hit.baseQuantity,
          actorData,
          powerInnerData,
          weaponInnerData,
          depth - 1,
        ...
        if (diceType.includes("weapon")) {
          ...
          for (let i = 0; i < parts.length; i++) {
            ...
            let weaponDiceQuantity = this.commonReplace(
              `(${quantity}) * (${parts[i][0]})`,
              actorData,
              powerInnerData,
              weaponInnerData,
    ...
    if (actorData?.effects && depth > 0 && newFormula.includes("@")) {
      ...
      if (debug) {
        console.log(
          `${debug} Substituting '${formula}', end of processing produced '${newFormula}' which still contains an @variable.	Searching active effects for a suitable variable`,
        );
      }
      const resultObject = {};
      const effects = actorData
      ...
      effects.forEach((effect) => {
        effect.changes.forEach((change) => {
          if (this.variableRegex.test(change.key)) {
            if (debug) {
              console.log(
                `${debug} Found custom variable ${change.key} in effect ${effect.name}.	Value: ${change.value}`,
              );
            }
            const changeValueReplaced = this.commonReplace(
              change.value,
              actorData,
              powerInnerData,
              weaponInnerData,
              0,
            ); // set depth to avoid infinite recursion
            if (!resultObject[change.key]) {
              resultObject[change.key] = changeValueReplaced;
              if (debug) {
                console.log(
                  `${debug} Effect: ${effect.name}.	Computed Value: ${change.value} was the first match to ${change.key} `,
                );
              }
            } else {
              if (debug) {
                console.log(
                  `${debug} Effect: ${effect.name}. Computed Value: ${change.value} was an additional match to ${change.key} adding to previous`,
                );
              }
              if (
                this._isNumber(resultObject[change.key]) &&
                this._isNumber(changeValueReplaced)
              ) {
                resultObject[change.key] =
                  Number(resultObject[change.key]) +
                  Number(changeValueReplaced);
              } else {
                resultObject[change.key] =
                  `${resultObject[change.key]} + ${changeValueReplaced}`;
              }
      ...
      if (debug) {
        console.log(
          `${debug} Discovered custom variable values in effects to substitute into formula (${newFormula}): ${JSON.stringify(resultObject)}`,
        );
      }
   ...
   */
  static async rollWithErrorHandling(
    rollString,
    { errorMessageKey = "DND4E.InvalidRollExpression", context = "" },
  ) {
    if (!errorMessageKey) {
    ...
    if (rollString && rollString !== "") {
      ...
      return roll.roll().catch((err) => {
        let msg = context
          ? `${game.i18n.localize(errorMessageKey)} (in ${context}) : ${rollString}`
          : `${game.i18n.localize(errorMessageKey)} : ${rollString}`;
  ...
  static _preparePowerCardData(
    ...
    let powerSource =
      chatData.powersource && chatData.powersource !== ""
        ? `${CONFIG.DND4E.powerSource[`${chatData.powersource}`]}`
        : "";
    ...
    if (
      chatData.powersource &&
      chatData.secondPowersource &&
      chatData.secondPowersource != chatData.powersource
    ) {
    ...
    if (chatData.weaponDamageType) {
      for (let [damage, d] of Object.entries(chatData.weaponDamageType)) {
        if (d && CONFIG.DND4E.damageTypes[damage])
          tag.push(CONFIG.DND4E.damageTypes[damage]);
      }
    ...
    if (chatData.effectType) {
      for (let [effect, e] of Object.entries(chatData.effectType)) {
        if (e && CONFIG.DND4E.effectTypes[effect])
          tag.push(CONFIG.DND4E.effectTypes[effect]);
      }
    }
    tag.sort();
    if (tag.length > 0)
      powerDetail += ` ♦ <span class="keywords">${tag.join(", ")}</span>`;
    ...
    if (chatData.rangeType === "weapon") {
      ...
      if (chatData.rangePower)
        powerDetail += ` <span class="range-value">${chatData.rangePower}</span>`;
      ...
      ["rangeBurst", "rangeBlast", "wall"].includes(chatData.rangeType)
    ) {
    ...
    if (chatData.target && typeof chatData.target === "string") {
      //target can sometimes be an object for things that did not have a dropdown
    ...
    if (chatData.attack.isAttack) {
      ...
      attackForm = chatData.attack.formula.replaceAll(
        "@powerMod",
        `@${chatData.attack?.ability}Mod`,
      );
    ...
    if (
      chatData.sustain?.actionType !== "none" &&
      chatData.sustain?.actionType
    ) {
  ...
  static async rechargeItems(actor, targetArray) {
    const items = actor.items.filter((item) =>
      targetArray.includes(item.system.uses?.per),
    );
    ...
    for (let effect of effectMap) {
      ...
      if (e.flags.dnd4e.effectData.powerEffectTypes === condition) {
        for (let t of tokenTarget) {
          ...
          flags.dnd4e.effectData.startTurnInit =
            combat?.turns[combat?.turn]?.initiative || 0;
          ...
          if (
            flags.dnd4e.effectData.durationType === "endOfTargetTurn" ||
            flags.dnd4e.effectData.durationType === "startOfTargetTurn"
          ) {
            duration.rounds = combat
              ? currentInit > targetInit
                ? combat.round
                : combat.round + 1
              : 0;
            ...
            duration.rounds = combat
              ? currentInit > userInit
                ? combat.round
                : combat.round + 1
              : 0;
            flags.dnd4e.effectData.durationTurnInit = userInit;
          } else if (
            flags.dnd4e.effectData.durationType === "endOfUserCurrent"
          ) {
          ...
          if (t?.actor) {
            ...
            //extra condition for when actors this linked data target self
            actor = parent;
          }
  ...
  static async applyAllXEffectsToTokens(effects, actor, selection) {
    if (selection?.size) {
      ...
      const parentDisposition =
        actor.token?.disposition || actor.prototypeToken.disposition || null;
      await this.applyEffectsToTokens(
        effects,
        this.filterActorSetByDisposition(selection, parentDisposition),
        "allies",
        actor,
      );
      await this.applyEffectsToTokens(
        effects,
        this.filterActorSetByDisposition(selection, parentDisposition, false),
        "enemies",
        actor,
      );
    }
  ...
  static isRollFastForwarded(event) {
    ...
    return game.settings.get("dnd4e", "fastFowardSettings")
      ? !isModKeyPressed
      : isModKeyPressed;
  }
  ...
  static hasEffects(power, effects) {
    const foundEffects = power.item.effects.contents.filter((e) =>
      effects.includes(e.flags.dnd4e.effectData.powerEffectTypes),
    );
...
export async function handleApplyEffectToToken(data) {
  ...
  const actor = data.tokenID
    ? game.scenes.get(data.scene).tokens.get(data.tokenID).actor
...
Handlebars.registerHelper("getSourceName", function (effect) {
  return effect.sourceName === "Unknown"
    ? effect.parent.name
    : effect.sourceName;
});
...
export function registerHandlebarsHelpers() {
  Handlebars.registerHelper({
    ...
    "DND4E-numberFormat": (context, options) =>
      formatNumber(context, options.hash),
...
export function getHumanReadableAttributeLabel(attr, { actor } = {}) {
  ...
  // Abilities.
    ...
    label = game.i18n.format("DND4E.AbilityScoreL", {
      ability: CONFIG.DND4E.abilities[key].label,
    });
  }
  ...
  // Skills.
    ...
    const [, key] = attr.split(".");
    label = game.i18n.format("DND4E.SkillPassiveScore", {
      skill: CONFIG.DND4E.skills[key].label,
    });
  }
  ...
  // Spell slots.
    ...
    if (!/spell\d+/.test(key)) label = `DND4E.SpellSlots${key.capitalize()}`;
      ...
      label = game.i18n.format(`DND4E.SpellSlotsN.${plurals.select(level)}`, {
        n: level,
      });
    }
  }
  ...
  if (!label) {
    const { CharacterData, NPCData, VehicleData, GroupData } =
      DND4E.dataModels.actor;
