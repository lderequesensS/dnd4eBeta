...
export default class Item4e extends Item {
  ...
  async _preUpdate(changed, options, user) {
    ...
    if (this.system.type === "consumable") {
      ...
      if (system.damage.parts?.length > 0) {
        console.log(
          "DnD4e: Updating an obsolete consumable that somehow still had a parts roll",
        );
        // ok so need to fix it
        if (
          system.damage.parts.map((d) => d[1]).includes("healing") &&
          !changed.system.hit?.healFormula
        ) {
          foundry.utils.setProperty(
            changed,
            "system.hit.healFormula",
  ...
  _onCreationName(data) {
    ...
    let newName = game.i18n.format("DND4E.ItemNew", {
  ...
  static getDefaultArtwork(itemData) {
    return (
      { img: CONFIG.DND4E.defaultArtwork.Item[itemData.type] } ??
      super.getDefaultArtwork(itemData)
    );
  }
  ...
  _onCreateOwnedEquipment(data, isNPC) {
    ...
    if (data.system.armour?.type === "armour") {
      if (actorProfs.value.includes(data.system.armourBaseType)) {
        updates["system.proficient"] = actorProfs.value.includes(
          data.system.armourBaseType,
        );
      } else if (data.system.armourBaseType === "custom") {
        updates["system.proficient"] = actorProfs.custom
          .split(";")
          .includes(data.system.armourBaseTypeCustom);
    ...
    if (
      data.system.armour?.type === "arms" &&
      CONFIG.DND4E.shield[data.system.armour.subType]
    ) {
      if (actorProfs.value.includes(data.system.shieldBaseType)) {
        updates["system.proficient"] = actorProfs.value.includes(
          data.system.shieldBaseType,
        );
      } else if (data.system.shieldBaseType === "custom") {
        updates["system.proficient"] = actorProfs.custom
          .split(";")
          .includes(data.system.shieldBaseTypeCustom);
      }
    }
  ...
  _onCreateOwnedWeapon(data, isNPC) {
    ...
    if (actorProfs.value.includes(data.system.weaponType)) {
      ...
      updates["system.proficient"] = actorProfs.custom
        .split(";")
        .includes(data.system.weaponBaseTypeCustom);
    } else {
      updates["system.proficient"] = actorProfs.value.includes(
        data.system.weaponBaseType,
      );
    }
    return updates;
  }
      ...
      // Weapons
        ...
        if (["simpleM", "martialM"].includes(wt)) {
          if (itemData.properties.fin === true) {
            // Finesse weapons
            return actorData.abilities["dex"].mod >=
              actorData.abilities["str"].mod
              ? "dex"
              : "str";
          }
          return "str";
  ...
  get hasDamage() {
    ...
    return (
      !!this.system.hit?.formula ||
      !!(this.system.damage && this.system.damage.parts.length)
    );
  }
  ...
  get hasAreaTarget() {
    return [
      ...
      "rangeBlast",
      "wall",
    ].includes(this.system.rangeType);
  }
  ...
  prepareData() {
    ...
    if (itemData.type === "equipment") {
      labels.armour = system.armour.ac
        ? `${system.armour.ac} ${game.i18n.localize("DND4E.AC")}`
        : "";
      labels.fort = system.armour.fort
        ? `${system.armour.fort} ${game.i18n.localize("DND4E.FORT")}`
        : "";
      labels.ref = system.armour.ref
        ? `${system.armour.ref} ${game.i18n.localize("DND4E.REF")}`
        : "";
      labels.wil = system.armour.wil
        ? `${system.armour.wil} ${game.i18n.localize("DND4E.WIL")}`
        : "";
    }
    ...
    if (system.hasOwnProperty("activation")) {
      ...
      labels.duration = dur.value
        ? `${game.i18n.localize("DND4E.Duration")}: ${[dur.value, C.timePeriods[dur.units]].filterJoin(" ")}`
        : null;
        ...
        if (system.consume?.amount && system.consume?.type === "attribute") {
          ...
          if (DND4E.ritualcomponents[resourceTarget]) {
            resourceLabel = game.i18n.localize(
              DND4E.ritualcomponents[resourceTarget],
            );
          } else if (DND4E.currencyConversion[resourceTarget]) {
            resourceLabel = game.i18n.localize(
              DND4E.currencies[resourceTarget],
            );
  ...
  async _computeEncumbrance() {
    ...
    if (type === "weight") {
      context.value = this.pack
        ? await this.contentsWeight
        : await this.contentsWeight.toNearest(0.01);
      context.units = game.i18n.localize("DND4E.AbbreviationLbs");
    } else {
      context.value = this.pack
        ? await this.contentsWeight
        : await this.contentsCount.toNearest(0.01);
    ...
    context.pec = Math.clamp(
      (context.value / context.max) * 100 - 100,
      1,
      99.7,
    );
  ...
  async roll({
    ...
    const cardData = await (async () => {
      if (
        (this.type === "power" || this.type === "consumable") &&
        this.system.autoGenChatPowerCard
      ) {
        ...
        let cardString = Helper._preparePowerCardData(
          await this.getChatData(),
          CONFIG,
          this.actor,
          attackBonus,
        );
        return Helper.commonReplace(
          cardString,
          this.actor,
          this,
          weaponUse ? weaponUse.system : null,
          1,
        );
      } else {
        return null;
    ...
    if (["tool", "ritual"].includes(this.type)) {
      ...
      templateData.abilityCheck = Helper.byString(
    ...
    if (["power", "consumable"].includes(templateData.item.type)) {
      html = html.replace(
        "ability-usage--",
        `ability-usage--${templateData.system.useType}`,
      );
      if (game.settings.get("dnd4e", "autoApplyEffects")) {
        Helper.applyEffectsToTokens(
          this.effects,
          [this.parent.token],
          "self",
          this.parent,
      ...
      ["weapon", "equipment", "backpack", "tool", "loot"].includes(
        templateData.item.type,
      )
    ) {
   ...
   * @private
   */
  async _handleResourceConsumption(
    { isCard = false, isAttack = false } = {},
    itemData,
  ) {
    // const itemData = this.system;
    ...
    const amount =
      parseInt(consume.amount) || parseInt(consume.amount) === 0
        ? parseInt(consume.amount)
        : 1;

    // Only handle certain types for certain actions
    if (
      (consume.type === "ammo" && !isAttack) ||
      (consume.type !== "ammo" && !isCard)
    )
      return true;
    ...
    if (!consume.target) {
      ui.notifications.warn(
        game.i18n.format("DND4E.ConsumeWarningNoResource", {
          name: this.name,
          type: typeLabel,
        }),
      );
    ...
    if ([null, undefined].includes(consumed)) {
      ui.notifications.warn(
        game.i18n.format("DND4E.ConsumeWarningNoSource", {
          name: this.name,
          type: typeLabel,
        }),
      );
    ...
    if (remaining < 0) {
      ui.notifications.warn(
        game.i18n.format("DND4E.ConsumeWarningNoQuantity", {
          name: this.name,
          type: typeLabel,
        }),
      );
  ...
  async _rollFeat(configureDialog) {
    ...
    if (consume && charge.value) {
      if (!charge.charged) {
        ui.notifications.warn(
          game.i18n.format("DND4E.ItemNoUses", { name: this.name }),
        );
        return false;
  ...
  async getChatData(htmlOptions = {}) {
    ...
    if (
      ...
      props.push(
        game.i18n.localize(
          data.equipped ? "DND4E.Equipped" : "DND4E.Unequipped",
        ),
        game.i18n.localize(
          data.proficient ? "DND4E.Proficient" : "DND4E.NotProficient",
        ),
    ...
    if (data.hasOwnProperty("activation")) {
      props.push(
        ...
        labels.attribute,
        labels.target,
        data.isRanged && labels.range
          ? `${game.i18n.localize("DND4E.Range")}: ${labels.range}`
          : "",
        labels.castTime,
        labels.duration,
        labels.component,
  ...
  _equipmentChatData(data, labels, props) {
    props.push(
      ...
      labels.wil || null,
      data.stealth.value
        ? game.i18n.localize("DND4E.StealthDisadvantage")
        : null,
    );
  }
  ...
  _weaponChatData(data, labels, props) {
    props.push(CONFIG.DND4E.weaponTypes[data.weaponType]);
  }
  ...
  _consumableChatData(data, labels, props) {
    props.push(
      ...
      data.uses.value +
        "/" +
        data.preparedMaxUses +
        " " +
        game.i18n.localize("DND4E.Charges"),
    );
  ...
  _lootChatData(data, labels, props) {
    props.push(
      ...
      data.weight
        ? data.weight + " " + game.i18n.localize("DND4E.AbbreviationLbs")
        : null,
    );
  }
  ...
  async rollAttack(options = {}) {
    ...
    if (!this.hasAttack) {
      ui.notifications.error(
        "You may not place an Attack Roll with this Item.",
      );
      return null;
    }
    ...
    if (!!itemData.attack.formula) {
      parts.push(
        Helper.commonReplace(
          itemData.attack.formula,
          actorData,
          this.system,
          weaponUse ? weaponUse.system : null,
        ),
      );
      partsExpressionReplacements.push({
        value: itemData.attack.formula,
        target: parts[0],
      });
      // add the substitutions that were used in the expression to the data object for later
      options.formulaInnerData = Helper.commonReplace(
        itemData.attack.formula,
        actorData,
        this.system,
        weaponUse ? weaponUse.system : null,
    ...
    const handlePowerAndWeaponAmmoBonuses = (
      onHasBonus,
      consumable,
      resourceType,
    ) => {
      if (consumable?.type === "ammo") {
        if (
          Helper.isNonEmpty(consumable.target) &&
          Helper.isNonEmpty(consumable.amount)
        ) {
          ...
          if (ammo) {
            ...
            if (ammoCount && ammoCount - consumable.amount >= 0) {
              ...
              ui.notifications.warn(
                game.i18n.format(
                  "The {resourceType} requires {quantity} of '{target}' but the character only has {ammoCount}",
                  {
                    resourceType,
                    ammoCount,
                    target: ammo.name,
                    quantity: consumable.amount,
                  },
                ),
              );
            }
            ...
            ui.notifications.warn(
              game.i18n.format(
                "The {resourceType} requires a ammunition but none could be found on the character",
          ...
          if (!Helper.isNonEmpty(consumable.target)) {
            ui.notifications.warn(
              game.i18n.format(
                "The {resourceType} requires ammunition, but the type ('{target}') was empty",
                {
                  resourceType,
                  target: consumable.target,
                  quantity: consumable.amount,
                },
              ),
            );
          ...
          if (!Helper.isNonEmpty(consumable.quantity)) {
            ui.notifications.warn(
              game.i18n.format(
                "The {resourceType} requires ammunition, but the quantity ('{quantity}') was empty",
                {
                  resourceType,
                  target: consumable.target,
                  quantity: consumable.amount,
    ...
    handlePowerAndWeaponAmmoBonuses(
      powerHasAmmoWithBonus,
      itemData.consume,
      "power",
    );
    ...
    if (weaponUse) {
      ...
      handlePowerAndWeaponAmmoBonuses(
        weaponHasAmmoWithBonus,
        weaponUse.system.consume,
        "weapon used by the power",
      );
    }

    await Helper.applyEffects(
      [parts],
      rollData,
      actorData,
      this,
    ...
    if (weaponUse) {
      rollConfig.critical =
        itemData.weaponType === "implement"
          ? weaponUse.system.critRangeImp
          : weaponUse.system.critRange;
    }
    ...
    const allowed = await (this._handleResourceConsumption(
      { isCard: false, isAttack: true },
      this.system,
    ),
    weaponUse
      ? this._handleResourceConsumption(
          { isCard: false, isAttack: true },
          this.actor.items.get(weaponUse.id).system,
        )
      : true);
  ...
  async rollDamage({
    ...
    const messageData = {
      "flags.dnd4e.roll": { type: "damage", itemId: this.id },
    };
    ...
    let flavor = this.labels.damageTypes?.length
    ...
    const returnDamageRollAndOptionalType = (damageRoll, damageType) => {
      if (
        damageType &&
        damageType !==
          game.i18n.localize(game.dnd4e.config.damageTypes.damage) &&
        damageType !== game.i18n.localize("DND4E.None")
      ) {
    ...
    const secondaryPartsHelper = (formula, damageType) => {
      ...
      options.formulaInnerData = foundry.utils.mergeObject(
        options.formulaInnerData,
        Helper.commonReplace(
          formula,
          actorData,
          this.system,
      ...
      return returnDamageRollAndOptionalType(
        Helper.commonReplace(
          formula,
          actorData,
          this.system,
    ...
    const parts = itemData.damage.parts.map((d) =>
      secondaryPartsHelper(d[0], d[1]),
    );
    const partsMiss = itemData.damage.parts.map((d) =>
      secondaryPartsHelper(d[0], d[1]),
    );
    const partsCrit = itemData.damageCrit.parts.map((d) =>
      secondaryPartsHelper(d[0], d[1]),
    );

    // store the original expression formula that produced those formula
    const partsExpressionReplacement = parts.map((part) => {
      return { target: part, value: "@pow2ndryDamage" };
    });
    const partsMissExpressionReplacement = partsMiss.map((part) => {
      return { target: part, value: "@pow2ndryDamage" };
    });
    const partsCritExpressionReplacement = partsCrit.map((part) => {
      return { target: part, value: "@pow2ndryCritDamage" };
    });
    ...
    if (!!itemData.hit?.formula) {
      ...
      const secondaryDamageExpressionHelper = (
        oldParts,
        expressionParts,
        newPartsArr,
      ) => {
        const newParts = newPartsArr.map((d) => {
          ...
          const formula = Helper.commonReplace(
            d[0],
            actorData,
            this.system,
            weaponUse?.system,
        ...
        Array.prototype.push.apply(
          expressionParts,
          newParts.map((part) => {
            return { target: part, value: "@wep2ndryDamage" };
          }),
        );
      };
      ...
      if (weaponUse) {
        if (
          itemData.hit.formula.includes("@wepDamage") &&
          weaponUse.system.damage.parts.length > 0
        ) {
          secondaryDamageExpressionHelper(
            parts,
            partsExpressionReplacement,
            weaponUse.system.damage.parts,
          );
        }
        if (
          itemData.hit.critFormula.includes("@wepCritBonus") &&
          weaponUse.system.damageCrit.parts.length > 0
        ) {
          secondaryDamageExpressionHelper(
            partsCrit,
            partsCritExpressionReplacement,
            weaponUse.system.damageCrit.parts,
          );
        }

        if (
          itemData.hit.formula.includes("@impDamage") &&
          weaponUse.system.proficientI &&
          weaponUse.system.damageImp.parts.length > 0
        ) {
          secondaryDamageExpressionHelper(
            parts,
            partsExpressionReplacement,
            weaponUse.system.damageImp.parts,
          );
        }
        if (
          itemData.hit.critFormula.includes("@impCritBonus") &&
          weaponUse.system.proficientI &&
          weaponUse.system.damageCritImp.parts.length > 0
        ) {
          secondaryDamageExpressionHelper(
            partsCrit,
            partsCritExpressionReplacement,
            weaponUse.system.damageCritImp.parts,
          );
        }

        if (
          itemData.miss.formula.includes("@wepDamage") &&
          weaponUse.system.damage.parts.length > 0
        ) {
          secondaryDamageExpressionHelper(
            partsMiss,
        ...
        if (
          itemData.miss.formula.includes("@impDamage") &&
          weaponUse.system.proficientI &&
          weaponUse.system.damageImp.parts.length > 0
        ) {
          secondaryDamageExpressionHelper(
    ...
    if (weaponUse) {
      if (
        weaponUse.system.properties["ver"] &&
        weaponUse.system.weaponHand === "hTwo"
      ) {
        damageFormula += `+ 1`;
        critDamageFormula += `+ 1`;
    ...
    if (this._ammo) {
      ...
      if (
        !missDamageFormula.includes("@damageFormula") &&
        !missDamageFormula.includes("@halfDamageFormula")
      ) {
        partsMiss.push("@ammo");
      }

      rollData["ammo"] = this._ammo.system.damage.parts
        .map((p) => p[0])
        .join("+");
    ...
    if (weaponUse) {
      if (weaponUse._ammo) {
        ...
        rollData["ammoW"] = weaponUse._ammo.system.damage.parts
          .map((p) => p[0])
          .join("+");
    ...
    if (missDamageFormula.includes("@damageFormula")) {
      missDamageFormula = missDamageFormula.replace(
        "@damageFormula",
        Helper.bracketed(damageFormula),
      );
    }
    ...
    if (missDamageFormula.includes("@halfDamageFormula")) {
      missDamageFormula = missDamageFormula.replace(
        "@halfDamageFormula",
        Helper.bracketed(`${damageFormula}/2`),
      );
    }
    ...
    partsExpressionReplacement.unshift({
      target: parts[0],
      value: damageFormulaExpression,
    });
    partsCritExpressionReplacement.unshift({
      target: partsCrit[0],
      value: critDamageFormulaExpression,
    });
    partsMissExpressionReplacement.unshift({
      target: partsMiss[0],
      value: missDamageFormulaExpression,
    });
  ...
  rollHealing({
    ...
    const messageData = {
      "flags.dnd4e.roll": { type: "healing", itemId: this.id },
    };
    ...
    const partsExpressionReplacement = itemData.damage.parts.map((part) => {
      return { target: part[0], value: "@wep2ndryDamage" };
    });
    ...
    if (weaponUse) {
      if (
        itemData.hit.healFormula.includes("@wepDamage") &&
        weaponUse.system.damage.parts.length > 0
      ) {
        Array.prototype.push.apply(
          parts,
          weaponUse.system.damage.parts.map((d) => formulaHelper(d[0])),
        );
        Array.prototype.push.apply(
          partsExpressionReplacement,
          weaponUse.system.damage.parts.map((part) => {
            return { target: part[0], value: "@wep2ndryDamage" };
          }),
        );
      }
      ...
      if (
        itemData.hit.healFormula.includes("@impDamage") &&
        weaponUse.system.proficientI &&
        weaponUse.system.damageImp.parts.length > 0
      ) {
        Array.prototype.push.apply(
          parts,
          weaponUse.system.damageImp.parts.map((d) => formulaHelper(d[0])),
        );
        Array.prototype.push.apply(
          partsExpressionReplacement,
          weaponUse.system.damageImp.parts.map((part) => {
            return { target: part[0], value: "@wep2ndryDamage" };
          }),
        );
      }
    ...
    if (consume) {
      ...
      if (usesRecharge) await this.update({ "system.recharge.charged": false });
        ...
        // Case 2, reduce quantity
          ...
          await this.update({
            "system.quantity": q - 1,
            "system.uses.value": this.preparedMaxUses || 0,
          });
        }
  ...
  async rollRecharge() {
    ...
    if (success)
      promises.push(this.update({ "system.recharge.charged": true }));
  ...
  rollToolOrRitualCheck(rollType, titleKey, options = {}) {
    ...
    if (this.system.formula) {
      rollData[rollType] = Helper.commonReplace(
        this.system.formula.replace(
          "@attribute",
          Helper.byString(this.system.attribute, this.actor.system),
        ),
        this.actor.system,
        this.system,
      );
      ...
      rollData[rollType] =
        `1d20 + ${Helper.byString(this.system.attribute, this.actor.system)}`;
    ...
    const flavor = this.system.chatFlavor
      ? `${this.system.chatFlavor} (${label} check)`
      : `${this.name} - ${game.i18n.localize(titleKey)}  (${label} check)`;
    ...
    const rollConfig = foundry.utils.mergeObject(
      {
        ...
        messageData: {
          "flags.dnd4e.roll": { type: rollType, itemId: this.id },
        },
      },
      options,
    );
  ...
  static chatListeners(html) {
    html.on(
      "click",
      ".card-buttons button, .effects-tray button",
    ...
    html.on(
      ...
      this._onChatCardEffectCollapsibleToggleContent.bind(this),
    );
  }
  ...
  static _onChatCardEffectCollapsibleToggleContent(event) {
    ...
    if (event.target.closest(":is(.item-name, .collapsible) :is(a, button)"))
      return;
  ...
  static async _onChatCardAction(event) {
    ...
    if (!item) {
      return ui.notifications.error(
        game.i18n.format("DND4E.ActionWarningNoItem", {
    ...
    if (action === "attack") await item.rollAttack({ event });
      ...
      await item.rollHealing({ event, spellLevel });
    else if (action === "versatile")
      await item.rollDamage({ event, spellLevel, versatile: true });
    ...
    // Effects
      ...
      const effect = await fromUuid(
        button.closest("[data-uuid]")?.dataset.uuid,
      );
      const targets = game.settings.get("dnd4e", "applyEffectsToSelection")
        ? canvas.tokens.controlled
        : game.user.targets;
      Helper.applyEffectsToTokens(
        [effect],
        targets,
        effect.flags.dnd4e.effectData.powerEffectTypes,
        actor,
      );
    } else if (action === "effect")
      Helper.applyAllXEffectsToTokens(item.effects, actor, effectTargets);
    else if (action === "hitEffect")
      Helper.applyEffectsToTokens(item.effects, effectTargets, "hit", actor);
    else if (action === "missEffect")
      Helper.applyEffectsToTokens(item.effects, effectTargets, "miss", actor);
    else if (action === "hitOrMissEffect")
      Helper.applyEffectsToTokens(
        item.effects,
        effectTargets,
        "hitOrMiss",
  ...
  get contents() {
    ...
    if (parent.pack && !parent.isEmbedded) {
      ...
      return pack
        .getDocuments({ system: { container: parent.id } })
        .then((d) => new foundry.utils.Collection(d.map((d) => [d.id, d])));
    }

    // Otherwise use local document collection
    return (parent.isEmbedded ? parent.actor.items : game.items).reduce(
      (collection, item) => {
  ...
  get container() {
    ...
    if (this.pack)
      return game.packs.get(this.pack).getDocument(this.system.container);
  ...
  getContainedItem(id) {
    ...
    if (this.parent?.pack)
      return game.packs.get(this.parent.pack)?.getDocument(id);
  ...
  get allContainedItems() {
    ...
    return this.contents.reduce((collection, item) => {
      ...
      if (item.type === "container")
        item.system.allContainedItems.forEach((i) => collection.set(i.id, i));
      return collection;
    }, new foundry.utils.Collection());
  }
  ...
  async #allContainedItems() {
    return (await this.contents).reduce(async (promise, item) => {
      ...
      collection.set(item.id, item);
      if (item.type === "container")
        (await item.system.allContainedItems).forEach((i) =>
  ...
  get contentsCount() {
    ...
    if (items instanceof Promise)
      return items.then((items) => items.reduce(reducer, 0));
  ...
  async #contentsWeight() {
    ...
    return contents.reduce(
      async (weight, item) => (await weight) + (await item.system.totalWeight),
      this.currencyWeight,
    );
  }
  ...
  get totalWeight() {
    ...
    if (containedWeight instanceof Promise)
      return containedWeight.then((c) => this.system.weight + c);
  ...
  async _renderContainers({ formerContainer, ...rendering } = {}) {
    ...
    else if (this.parent?.pack)
      game.packs
        .get(this.parent.pack)
        .apps.forEach((a) => a.render(false, rendering));
  ...
  async allContainers() {
    ...
    while (
      (container = await item.container) &&
      depth < CONFIG.DND4E.PhysicalItemTemplate.MAX_DEPTH
    ) {
  ...
  static async createWithContents(
    items,
    { container, keepId = false, transformAll, transformFirst } = {},
  ) {
    let depth = 0;
    if (container) {
      ...
      if (depth > CONFIG.DND4E.PhysicalItemTemplate.MAX_DEPTH) {
        ui.notifications.warn(
          game.i18n.format("DND4E.ContainerMaxDepth", {
            depth: CONFIG.DND4E.PhysicalItemTemplate.MAX_DEPTH,
          }),
        );
        return;
    ...
    const createItemData = async (item, containerId, depth) => {
      ...
      if (transformFirst && depth === 0)
        newItemData = await transformFirst(newItemData);
