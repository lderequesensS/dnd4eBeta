...
export default class ItemSheet4e extends ItemSheet {
  ...
  async getData(options) {
    ...
    if (itemData.type === "power") {
      ...
      data.effectsPowers = this._prepareEffectPowersCategories(
        this.item.effects,
      );
    }
    ...
    if (itemData.type === "equipment") {
      data.equipmentSubTypeTargets = this._getItemEquipmentSubTypeTargets(
        itemData,
        data.config,
      );
      ...
      if (itemData.system.armour.type === "armour") {
        ...
        data.armourBaseTypes = CONFIG.DND4E[itemData.system.armour.subType] || {
          "": game.i18n.localize("DND4E.None"),
        };
        data.isArmourBaseTypeCustom =
          itemData.system.armourBaseType === "custom";
      } else if (
        itemData.system.armour.type === "arms" &&
        CONFIG.DND4E.profArmor[itemData.system.armour.subType]
      ) {
        data.isShield = true;
        data.hasBaseProps = true;
        data.shieldBaseTypes = CONFIG.DND4E.shield;
        data.isShieldBaseTypeCustom =
          itemData.system.shieldBaseType === "custom";
    ...
    if (itemData.system?.useType) {
      if (
        !(
          itemData.system.rangeType === "personal" ||
          itemData.system.rangeType === "closeBurst" ||
          itemData.system.rangeType === "closeBlast" ||
          itemData.system.rangeType === ""
        )
      ) {
      ...
      if (
        itemData.system.rangeType === "closeBurst" ||
        itemData.system.rangeType === "closeBlast" ||
        itemData.system.rangeType === "rangeBurst" ||
        itemData.system.rangeType === "rangeBlast" ||
        itemData.system.rangeType === "wall"
      ) {
    ...
    if (itemData.type === "weapon") {
      ...
      for (let attrib in data.config.weaponProperties) {
        data.weaponMetaProperties[attrib] = {
          ...
          disabled:
            attrib === "imp" && itemData.system.weaponType === "implement",
        };
      }
    ...
    data.isFlatDC =
      foundry.utils.getProperty(itemData.system, "save.scaling") === "flat";
    ...
    const weaponUse = this.actor
      ? Helper.getWeaponUse(itemData.system, this.actor)
      : null;
    ...
    const descriptionTextGM = data.system.description.gm
      ? Helper.commonReplace(
          descriptionGM,
          itemActor,
          itemData.system,
    ...
    data.descriptionHTMLGM = await TextEditor.enrichHTML(
      descriptionTextGM || descriptionGM,
      {
        secrets: data.item.isOwner,
        async: true,
        relativeTo: this.item,
    ...
    data.effectDetailHTML = await TextEditor.enrichHTML(
      data.system.effect?.detail,
      {
        secrets: data.item.isOwner,
        async: true,
        relativeTo: this.item,
  ...
  _prepareEffectPowersCategories(effectPowers) {
    ...
    if (effectPowers) {
      for (let e of effectPowers) {
        ...
        if (e.flags.dnd4e?.effectData?.powerEffectTypes === "hit")
          categories.hit.effects.push(e);
        else if (e.flags.dnd4e?.effectData?.powerEffectTypes === "miss")
          categories.miss.effects.push(e);
        else if (e.flags.dnd4e?.effectData?.powerEffectTypes === "hitOrMiss")
          categories.hitOrMiss.effects.push(e);
        else if (e.flags.dnd4e?.effectData?.powerEffectTypes === "self")
          categories.self.effects.push(e);
        else if (e.flags.dnd4e?.effectData?.powerEffectTypes === "selfHit")
          categories.selfHit.effects.push(e);
        else if (e.flags.dnd4e?.effectData?.powerEffectTypes === "selfMiss")
          categories.selfMiss.effects.push(e);
        else if (
          e.flags.dnd4e?.effectData?.powerEffectTypes === "selfAfterAttack"
        )
          categories.selfAfterAttack.effects.push(e);
        else if (e.flags.dnd4e?.effectData?.powerEffectTypes === "allies")
          categories.allies.effects.push(e);
        else if (e.flags.dnd4e?.effectData?.powerEffectTypes === "enemies")
          categories.enemies.effects.push(e);
        else if (e.flags.dnd4e?.effectData?.powerEffectTypes === "all")
          categories.all.effects.push(e);
  ...
  async shareItem() {
    ...
    if (this.item.ownership.default <= 0) {
      ...
      changeBack = !game.keyboard.downKeys.has(
        game.keybindings.bindings.get(`dnd4e.permShowPlayer`)[0].key,
      );
    }
  ...
  _getItemEquipmentSubTypeTargets(item, config) {
    if (item.system.armour.type == "armour") {
      return config.equipmentTypesArmour;
    } else if (item.system.armour.type == "arms") {
      return config.equipmentTypesArms;
    } else if (item.system.armour.type == "feet") {
      return config.equipmentTypesFeet;
    } else if (item.system.armour.type == "hands") {
      return config.equipmentTypesHands;
    } else if (item.system.armour.type == "head") {
      return config.equipmentTypesHead;
    } else if (item.system.armour.type == "neck") {
      return config.equipmentTypesNeck;
    } else if (item.system.armour.type == "ring") {
      return null;
    } else if (item.system.armour.type == "waist") {
      return config.equipmentTypesWaist;
    } else if (item.system.armour.type == "natural") {
      return null;
    } else if (item.system.armour.type == "other") {
      return null;
    }
  ...
  _getItemConsumptionTargets(item) {
    ...
    if (consume.type === "attribute") {
      if (actor) {
        ...
        return {
          "": game.i18n.localize("DND4E.None"),
          ...attributes.bar.concat(attributes.value).reduce((obj, a) => {
            let k = a.join(".");
            obj[k] = k;
            return obj;
          }, {}),
        ...
        const attributeKeys = Object.keys(
          foundry.utils.flattenObject(attributes),
        ).reduce((obj, a) => {
          obj[a] = a;
          return obj;
        }, {});
    ...
    // Ammunition
      ...
      return {
        "": game.i18n.localize("DND4E.None"),
        ...actor.itemTypes.consumable.reduce((ammo, i) => {
  ...
  _getItemStatus(item) {
    if (item.type === "spell") {
      ...
      return game.i18n.localize(
        item.system.equipped ? "DND4E.Equipped" : "DND4E.Unequipped",
      );
    } else if (item.type === "tool") {
      return game.i18n.localize(
        item.system.proficient ? "DND4E.Proficient" : "DND4E.NotProficient",
      );
    }
  }
  ...
  _getItemProperties(item) {
    ...
    if (item.type === "weapon") {
      ...
      props.push(
        ...Object.entries(item.system.properties)
          ...
          .map((e) => {
            if (e[0] === "bru")
              return `${CONFIG.DND4E.weaponProperties[e[0]]} ${item.system.brutalNum}`;
            return CONFIG.DND4E.weaponProperties[e[0]];
          }),
      );
      props.push(
        ...Object.entries(item.system.damageType)
          .filter((e) => e[1] === true)
          .map((e) => CONFIG.DND4E.damageTypes[e[0]]),
      );

      props.push(
        ...Object.entries(item.system.weaponGroup)
          .filter((e) => e[1] === true)
      ...
      if (item.system.isRanged)
        props.push(
          `${game.i18n.localize("DND4E.Range")}: ${item.system.range.value} / ${item.system.range.long}`,
        );
    } else if (
      item.type === "power" ||
      ["power", "atwill", "encounter", "daily", "utility", "item"].includes(
        item.system.type,
      )
    ) {
      props.push(
    ...
    if (
      ...
      !foundry.utils.isEmpty(item.system.activation)
    ) {
      props.push(
        labels.attribute,
        labels.activation,
  ...
  _isItemMountable(item) {
    ...
    return (
      (item.type === "weapon" && system.weaponType === "siege") ||
  ...
  _updateObject(event, formData) {
    ...
    if (damageCrit)
      damageCrit.parts = Object.values(damageCrit?.parts || {}).map((d) => [
        d[0] || "",
        d[1] || "",
      ]);
    ...
    if (damageImp)
      damageImp.parts = Object.values(damageImp?.parts || {}).map((d) => [
        d[0] || "",
        d[1] || "",
      ]);
    const damageCritImp = formData.system?.damageCritImp;
    if (damageCritImp)
      damageCritImp.parts = Object.values(damageCritImp?.parts || {}).map(
        (d) => [d[0] || "", d[1] || ""],
      );

    const damageRes = formData.system.armour?.damageRes;
    if (damageRes)
      damageRes.parts = Object.values(damageRes?.parts || {}).map((d) => [
        d[0] || "",
        d[1] || "",
      ]);
    ...
    if (damageDice)
      damageDice.parts = Object.values(damageDice?.parts || {}).map((d) => [
        d[0] || "",
        d[1] || "",
        d[2] || "",
      ]);
    ...
    if (special)
      special.parts = Object.values(special?.parts || {}).map((d) => [d || ""]);
  ...
  activateListeners(html) {
    ...
    if (this.isEditable) {
      ...
      html
        .find(".powereffect-control")
        .click(this._onPowerEffectControl.bind(this));
    }
  ...
  async _onDamageControl(event) {
    ...
    if (a.classList.contains("add-criticalDamage")) {
      ...
      return this.item.update({
        "system.damageCrit.parts": damageCrit.parts.concat([["", ""]]),
      });
    }
    ...
    if (a.classList.contains("add-damage-imp")) {
      ...
      return this.item.update({
        "system.damageImp.parts": damageImp.parts.concat([["", ""]]),
      });
    }
    ...
    if (a.classList.contains("add-criticalDamage-imp")) {
      ...
      return this.item.update({
        "system.damageCritImp.parts": damageCritImp.parts.concat([["", ""]]),
      });
    }
    ...
    if (a.classList.contains("delete-criticalDamage-imp")) {
      ...
      return this.item.update({
        "system.damageCritImp.parts": damageCritImp.parts,
      });
    }
    // Add new damage res
    if (a.classList.contains("add-damageRes")) {
      ...
      return this.item.update({
        "system.armour.damageRes.parts": damageRes.parts.concat([["", ""]]),
      });
    }
    ...
    if (a.classList.contains("delete-damageRes")) {
      ...
      return this.item.update({
        "system.armour.damageRes.parts": damageRes.parts,
      });
    }
    ...
    if (a.classList.contains("add-dice")) {
      ...
      return this.item.update({
        "system.damageDice.parts": damageDice.parts.concat([["", "", ""]]),
      });
    }
  ...
  async _onOnetextControl(event) {
    ...
    if (a.classList.contains("add-special")) {
      ...
      return this.item.update({
        "system.specialAdd.parts": special.parts.concat([[""]]),
      });
    }
  ...
  _onConfigureClassSkills(event) {
    ...
    const choices =
      skills.choices && skills.choices.length
        ? skills.choices
        : Object.keys(CONFIG.DND4E.skills);
    const a = event.currentTarget;
