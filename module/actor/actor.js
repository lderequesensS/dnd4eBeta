...
export class Actor4e extends Actor {
  ...
  async update(data, options = {}) {
    ...
    if (
      ...
      options.dhp =
        data[`system.attributes.hp.value`] - this.system.attributes.hp.value;
      data[`system.details.isBloodied`] =
        data[`system.attributes.hp.value`] <= this.system.attributes.hp.max / 2;
    }
    ...
    if (
      newSize &&
      (options.forceSizeUpdate === true ||
        newSize !== foundry.utils.getProperty(this, "system.details.size"))
    ) {
    ...
    if (data[`system.details.level`]) {
      if (
        this.system.details.tier !=
        Math.clamp(
          Math.floor((data[`system.details.level`] - 1) / 10 + 1),
          1,
          3,
        )
      ) {
        this.system.details.tier = Math.clamp(
          Math.floor((data[`system.details.level`] - 1) / 10 + 1),
          1,
          3,
        );
    ...
    for (let [id, abl] of Object.entries(this.system.abilities)) {
      if (data[`system.abilities.${id}.value`]) {
        if (
          this.system.abilities[id].mod !=
          Math.floor((data[`system.abilities.${id}.value`] - 10) / 2)
        ) {
          data[`system.abilities.${id}.mod`] = Math.floor(
            (data[`system.abilities.${id}.value`] - 10) / 2,
          );
        }
      }
    }
  ...
  getRollData() {
    ...
    data["paragon"] =
      data.details.level >= 11 && data.details.level < 21 ? 1 : 0;
  ...
  prepareBaseData() {
    ...
    const bonuses =
      foundry.utils.getProperty(system, "bonuses.abilities") || {};

    // Ability modifiers and saves
    // Character All Ability Check" and All Ability Save bonuses added when rolled since not a fixed value.
    const saveBonus = Number.isNumeric(bonuses.save)
      ? parseInt(bonuses.save)
      : 0;
    const checkBonus = Number.isNumeric(bonuses.check)
      ? parseInt(bonuses.check)
      : 0;
    ...
    if (
      (system.defences.ac.light || this.checkLightArmour()) &&
      system.defences.ac.altability !== "none"
    ) {
      system.defences.ac.ability =
        system.abilities.dex.value >= system.abilities.int.value
          ? "dex"
          : "int";
    ...
    system.defences.fort.ability =
      system.abilities.str.value >= system.abilities.con.value ? "str" : "con";
    system.defences.ref.ability =
      system.abilities.dex.value >= system.abilities.int.value ? "dex" : "int";
    system.defences.wil.ability =
      system.abilities.wis.value >= system.abilities.cha.value ? "wis" : "cha";
  }
  ...
  prepareDerivedData() {
    ...
    if (system.attributes.hp.autototal) {
      system.attributes.hp.max =
        system.attributes.hp.perlevel * (system.details.level - 1) +
        system.attributes.hp.starting +
        system.attributes.hp.misc +
    ...
    if (
      !(
        system.details.surgeBon.bonus.length === 1 &&
        jQuery.isEmptyObject(system.details.surgeBon.bonus[0])
      )
    ) {
    ...
    if (
      !(
        system.details.secondwindbon.bonus.length === 1 &&
        jQuery.isEmptyObject(system.details.secondwindbon.bonus[0])
      )
    ) {
    ...
    system.details.secondwindbon.value +=
      system.details.secondwindbon.feat || 0;
    system.details.secondwindbon.value +=
      system.details.secondwindbon.item || 0;
    system.details.secondwindbon.value +=
      system.details.secondwindbon.power || 0;
    system.details.secondwindbon.value +=
      system.details.secondwindbon.race || 0;
    system.details.secondwindbon.value +=
      system.details.secondwindbon.untyped || 0;

    system.details.bloodied = Math.floor(system.attributes.hp.max / 2);
    system.details.surgeValue =
      Math.floor(system.details.bloodied / 2) + system.details.surgeBon.value;
    system.attributes.hp.min = -system.details.bloodied;
    system.details.secondWindValue =
      system.details.surgeValue + system.details.secondwindbon.value;

    //check if bloodied
    system.details.isBloodied =
      system.attributes.hp.value <= system.attributes.hp.max / 2;

    if (
      !(
        system.details.surgeEnv.bonus.length === 1 &&
        jQuery.isEmptyObject(system.details.surgeEnv.bonus[0])
      )
    ) {
    ...
    if (
      !(
        system.details.saves.bonus.length === 1 &&
        jQuery.isEmptyObject(system.details.saves.bonus[0])
      )
    ) {
    ...
    if (
      !(
        system.details.deathsavebon.bonus.length === 1 &&
        jQuery.isEmptyObject(system.details.deathsavebon.bonus[0])
      )
    ) {
    ...
    system.details.deathsavebon.value +=
      system.details.deathsavebon.untyped || 0;
    ...
    system.modifiers.attack.warn =
      system.modifiers.attack.armourPen < 0 ? true : false;
    ...
    if (
      !(
        system.attributes.init.bonus.length === 1 &&
        jQuery.isEmptyObject(system.attributes.init.bonus[0])
      )
    ) {
    ...
    if (this.type === "NPC" && !system.advancedCals) {
      system.attributes.init.value =
        (system.attributes.init.ability
          ? system.abilities[system.attributes.init.ability].mod
          : 0) +
        (system.attributes.init.base || 0) +
        initBonusValue;
    } else {
      system.attributes.init.value = system.attributes.init.ability
        ? system.abilities[system.attributes.init.ability].mod + initBonusValue
        : initBonusValue;
    }
    ...
    if (system.attributes.init.value > 999) system.attributes.init.value = 999;
    ...
    if (
      !(
        system.movement.base.bonus.length === 1 &&
        jQuery.isEmptyObject(system.movement.base.bonus[0])
      )
    ) {
    ...
    if (
      !(
        system.movement.walk.bonus.length === 1 &&
        jQuery.isEmptyObject(system.movement.walk.bonus[0])
      )
    ) {
    ...
    if (
      !(
        system.movement.charge.bonus.length === 1 &&
        jQuery.isEmptyObject(system.movement.charge.bonus[0])
      )
    ) {
    ...
    if (
      !(
        system.movement.run.bonus.length === 1 &&
        jQuery.isEmptyObject(system.movement.run.bonus[0])
      )
    ) {
    ...
    if (
      !(
        system.movement.climb.bonus.length === 1 &&
        jQuery.isEmptyObject(system.movement.climb.bonus[0])
      )
    ) {
    ...
    if (
      !(
        system.movement.shift.bonus.length === 1 &&
        jQuery.isEmptyObject(system.movement.shift.bonus[0])
      )
    ) {
    ...
    system.movement.base.value =
      system.movement.base.base +
      baseMoveBonusValue +
      system.movement.base.temp;
    ...
    let walkForm = eval(
      Helper.replaceData(
        system.movement.walk.formula
          .replace(/@base/g, system.movement.base.value)
          .replace(/@armour/g, system.movement.base.armour),
        system,
      ).replace(/[^-()\d/*+. ]/g, ""),
    );
    system.movement.walk.value +=
      walkForm + walkBonusValue + system.movement.base.temp;
    ...
    if (system.movement.walk.value < 0) system.movement.walk.value = 0;

    let runForm = eval(
      Helper.replaceData(
        system.movement.run.formula
          .replace(/@base/g, system.movement.base.value)
          .replace(/@armour/g, system.movement.base.armour),
        system,
      ).replace(/[^-()\d/*+. ]/g, ""),
    );
    system.movement.run.value =
      runForm + runBonusValue + system.movement.run.temp;
    ...
    if (system.movement.run.value < 0) system.movement.run.value = 0;

    let chargeForm = eval(
      Helper.replaceData(
        system.movement.charge.formula
          .replace(/@base/g, system.movement.base.value)
          .replace(/@armour/g, system.movement.base.armour),
        system,
      ).replace(/[^-()\d/*+. ]/g, ""),
    );
    system.movement.charge.value =
      chargeForm + chargeBonusValue + system.movement.charge.temp;
    ...
    if (system.movement.charge.value < 0) system.movement.charge.value = 0;

    let climbForm = eval(
      Helper.replaceData(
        system.movement.climb.formula
          .replace(/@base/g, system.movement.base.value)
          .replace(/@armour/g, system.movement.base.armour),
        system,
      ).replace(/[^-()\d/*+. ]/g, ""),
    );
    system.movement.climb.value =
      climbForm + climbBonusValue + system.movement.climb.temp;
    ...
    if (system.movement.climb.value < 0) system.movement.climb.value = 0;

    let shiftForm = eval(
      Helper.replaceData(
        system.movement.shift.formula
          .replace(/@base/g, system.movement.base.value)
          .replace(/@armour/g, system.movement.base.armour),
        system,
      ).replace(/[^-()\d/*+. ]/g, ""),
    );
    system.movement.shift.value =
      shiftForm + shiftBonusValue + system.movement.shift.temp;
    ...
    if (system.movement.shift.value < 0) system.movement.shift.value = 0;
      ...
      for (let [id, res] of Object.entries(system.resistances)) {
        ...
        if (!(res.bonus.length === 1 && jQuery.isEmptyObject(res.bonus[0]))) {
          for (const b of res.bonus) {
            ...
            let val = Helper._isNumber(b.value)
              ? b.value
              : Helper.replaceData(b.value, system);
        ...
        for (let i of this.items) {
          if (
            i.type != "equipment" ||
            !i.system.equipped ||
            i.system.armour.damageRes.parts.filter((p) => p[1] === id)
              .length === 0
          ) {
            continue;
          }
          res.armour += i.system.armour.damageRes.parts.filter(
            (p) => p[1] === id,
          )[0][0];
          break;
        }
    ...
    //Magic Items
    system.magicItemUse.perDay =
      Math.clamp(Math.floor((system.details.level - 1) / 10 + 1), 1, 3) +
      system.magicItemUse.bonusValue +
      system.magicItemUse.milestone;
  }
  ...
  defaultSecondWindEffect() {
    if (
      this.system.details.secondwindEffect &&
      Object.keys(this.system.details.secondwindEffect).length
    ) {
      return;
    }
  ...
  calcDefenceStatsCharacter(data) {
    ...
    try {
      ...
      console.warn(
        `PC global defence calc failed, probably due to an unmigrated actor. Defences will function but this bonus will not be correctly applied. (Error message: "${e}")`,
      );
      globalBonus = {
        class: 0,
        feat: 0,
    ...
    for (let [id, def] of Object.entries(data.defences)) {
      ...
      for (let i of this.items) {
        ...
        if (
          i.system.armour.type === "arms" &&
          ["light", "heavy"].includes(i.system.armour.subType)
        ) {
          if (!i.system.proficient) {
            continue;
          } //if not proficient with a shield you do not gain any of its benefits
          ...
          if (!i.system.proficient) {
            //if not proficient with armour you have -2 to Ref def and -2 to attack rolls
            def.armour -= 2;
            this.system.modifiers.attack.armourPen = -2;
          }
        } else if (
          (i.system.armour.type === "armour" && id === "ac") ||
          (i.system.armour.type === "neck" &&
            ["fort", "ref", "wil"].includes(id))
        ) {
      ...
      if (game.settings.get("dnd4e", "inhEnh")) {
        ...
        const enhFloor = Helper.findKeyScale(
          data.details.level,
          CONFIG.DND4E.SCALE.basic,
          3,
        );
  ...
  calcDefenceStatsNPC(data) {
    ...
    try {
      ...
      console.warn(
        `NPC global defence calc failed, probably due to an unmigrated actor. Defences will function but this bonus will not be correctly applied. (Error message: "${e}")`,
      );
      globalBonus = {
        class: 0,
        feat: 0,
    ...
    for (let [id, def] of Object.entries(data.defences)) {
      ...
      if (debug) {
        console.debug(
          `${def.label} of ${this.name} is calculated as ${def.value} (Advanced calcs ${data.advancedCals}, half-level ${game.settings.get("dnd4e", "halfLevelOptions")})`,
        );
      }
    }
  }
  ...
  calcSkillCharacter(system) {
    ...
    try {
      ...
      console.warn(
        `PC global skill calc failed, probably due to an unmigrated actor. Skills will function but this bonus will not be correctly applied. (Error message: "${e}")`,
      );
      globalBonus = {
        class: 0,
        feat: 0,
    ...
    for (const [id, skl] of Object.entries(system.skills)) {
      ...
      switch (skl.training) {
        case 8:
          trainingBonus =
            system.skillTraining.expertise.value +
            system.skillTraining.expertise.untyped;
          featBonus = Math.max(
            system.skillTraining.expertise.feat,
            skl.feat,
            0,
          );
          itemBonus = Math.max(
            system.skillTraining.expertise.item,
            skl.item,
            0,
          );
          powerBonus = Math.max(
            system.skillTraining.expertise.power,
            skl.power,
            0,
          );
          raceBonus = Math.max(
            system.skillTraining.expertise.race,
            skl.race,
            0,
          );
          break;
        case 5:
          trainingBonus =
            system.skillTraining.trained.value +
            system.skillTraining.trained.untyped;
          ...
          powerBonus = Math.max(
            system.skillTraining.trained.power,
            skl.power,
            0,
          );
        ...
        case 0:
          trainingBonus =
            system.skillTraining.untrained.value +
            system.skillTraining.untrained.untyped;
          featBonus = Math.max(
            system.skillTraining.untrained.feat,
            skl.feat,
            0,
          );
          itemBonus = Math.max(
            system.skillTraining.untrained.item,
            skl.item,
            0,
          );
          powerBonus = Math.max(
            system.skillTraining.untrained.power,
            skl.power,
            0,
          );
          raceBonus = Math.max(
            system.skillTraining.untrained.race,
            skl.race,
            0,
          );
      }
  ...
  calcSkillNPC(system) {
    ...
    try {
      ...
      console.warn(
        `NPC global skill calc failed, probably due to an unmigrated actor. Skills will function but this bonus will not be correctly applied. (Error message: "${e}")`,
      );
      globalBonus = {
        class: 0,
        feat: 0,
        ...
        switch (skl.training) {
          ...
          case 5:
            trainingBonus =
              ...
              system.skillTraining.trained.untyped;
            featBonus = Math.max(
              system.skillTraining.trained.feat,
              skl.feat,
              0,
            );
            itemBonus = Math.max(
              system.skillTraining.trained.item,
              skl.item,
              0,
            );
            powerBonus = Math.max(
            ...
            raceBonus = Math.max(
              system.skillTraining.trained.race,
              skl.race,
              0,
            );
            break;
  ...
  checkLightArmour() {
    for (let i of this.items) {
      ...
      if (
        i.system.armour.type === "armour" &&
        i.system.armour.subType === "heavy"
      ) {
        return false;
      }
    }
  ...
  setConditions(newValue) {
    ...
    if (newValue > this.system.attributes.hp.max)
      newValue = this.system.attributes.hp.max;
    else if (newValue < this.system.attributes.hp.min)
      newValue = this.system.attributes.hp.min;
  ...
  rollSkill(skillId, options = {}) {
    ...
    const bonuses =
      foundry.utils.getProperty(this.system, "bonuses.abilities") || {};
    ...
    return d20Roll(
      foundry.utils.mergeObject(options, {
        parts: parts,
        data: data,
        title: game.i18n.format("DND4E.SkillPromptTitle", {
          skill: CONFIG.DND4E.skills[skillId],
        }),
  ...
  rollAbility(abilityId, options = {}) {
    ...
    // Construct parts
    const parts = game.settings.get("dnd4e", "halfLevelOptions")
      ? ["@mod"]
      : ["@mod", "@halfLevel"];
    const data = game.settings.get("dnd4e", "halfLevelOptions")
      ? { mod: abl.mod }
      : { mod: abl.mod, halfLevel: Math.floor(this.system.details.level / 2) };
    ...
    let flavText = this.system.abilities[abilityId].chat.replace(
      "@name",
      this.name,
    );
    flavText = flavText.replace(
      "@label",
      this.system.abilities[abilityId].label,
    );
    ...
    return d20Roll(
      foundry.utils.mergeObject(options, {
        parts: parts,
        data: data,
        title: game.i18n.format("DND4E.AbilityPromptTitle", {
          ability: CONFIG.DND4E.abilities[label],
        }),
        speaker: ChatMessage.getSpeaker({ actor: this }),
        flavor: flavText,
  ...
  rollDef(defId, options = {}) {
    ...
    // Add global actor bonus
    const bonuses =
      foundry.utils.getProperty(this.system, "bonuses.defences") || {};
    if (bonuses.check) {
      parts.push("@checkBonus");
      data.checkBonus = bonuses.check;
    }
    ...
    return d20Roll(
      foundry.utils.mergeObject(options, {
        parts: parts,
        data: data,
        title: game.i18n.format("DND4E.DefencePromptTitle", {
          defences: CONFIG.DND4E.defensives[label].label,
        }),
        speaker: ChatMessage.getSpeaker({ actor: this }),
        flavor: flavText,
      }),
  ...
  async rollInitiative(
    ...
    const isReroll = !!(
      game.combat.combatants.get(combatants[0]).initiative ||
      game.combat.combatants.get(combatants[0]).initiative == 0
    );
    ...
    const rollConfig = foundry.utils.mergeObject(options, {
      ...
      speaker: ChatMessage.getSpeaker({ actor: this }),
      flavor: isReroll
        ? `${this.name} ${game.i18n.localize("DND4E.RollsInitReroll")}!`
        : `${this.name} ${game.i18n.localize("DND4E.RollsInit")}!`,
    ...
    if (combatants[0])
      game.combat.combatants
        .get(combatants[0])
        .update({ initiative: initRoll.total });
    return combat;
  }
  ...
  async rollSave(event, options) {
    ...
    rollConfig.critical =
      options.dc - this.system.details.saves.value - options.save || 10;
    rollConfig.fumble =
      options.dc - 1 - this.system.details.saves.value - options.save || 9;
  ...
  async rollDeathSave(event, options) {
    ...
    rollConfig.fumble =
      9 - options.save - this.system.details.deathsavebon.value;
    const roll = await d20Roll(rollConfig);

    if (roll.total < 10) {
      updateData[`system.details.deathsavefail`] =
        this.system.details.deathsavefail + 1;
    }
    if (
      roll.total < 10 &&
      this.system.details.deathsavefail + 1 >= this.system.details.deathsaves
    ) {
      ...
      await ChatMessage.create({
        ...
        speaker: ChatMessage.getSpeaker(),
        content:
          this.name + game.i18n.localize("DND4E.DeathSaveCriticalSuccess"),
      });
    }
  ...
  async shortRest(event, options) {
    ...
    if (options.surge > 0) {
        ...
        if (options.bonus != "") {
          ...
          try {
            ...
            ui.notifications.error(
              game.i18n.localize("DND4E.InvalidHealingBonus"),
            );
            r = new Roll("0");
            // await r.roll({async : true});
            await r.roll();
      ...
      if (this.system.details.surges.value > 0)
        updateData[`system.details.surges.value`] =
          this.system.details.surges.value - options.surge;
    }
    ...
    if (this.type === "Player Character") {
      ...
      ChatMessage.create({
        ...
        content:
          options.surge >= 1
            ? `${this.name} ${game.i18n.localize("DND4E.ShortRestChat")}, ${game.i18n.localize("DND4E.Spending")} ${options.surge} ${game.i18n.localize("DND4E.SurgesSpendRegain")} ${updateData[`system.attributes.hp.value`] - Math.max(0, this.system.attributes.hp.value)} ${game.i18n.localize("DND4E.HPShort")}.`
  ...
  async longRest(event, options) {
    ...
    if (options.envi == "false") {
      if (this.system.details.surgeEnv.value > this.system.details.surges.max) {
        ...
        updateData[`system.attributes.hp.value`] =
          this.system.attributes.hp.max +
          (this.system.details.surges.max -
            this.system.details.surgeEnv.value) *
            Math.floor(this.system.details.bloodied / 2);
      } else {
        updateData[`system.details.surges.value`] =
          this.system.details.surges.max - this.system.details.surgeEnv.value;
        updateData[`system.attributes.hp.value`] =
          this.system.attributes.hp.max;
      }
    ...
    updateData[`system.magicItemUse.dailyuse`] =
      this.system.magicItemUse.perDay;
  ...
  async secondWind(event, options) {
    let r = await Helper.rollWithErrorHandling(options.bonus, {
      errorMessageKey: "DND4E.InvalidHealingBonus",
    });
    ...
    if (this.system.attributes.hp.value <= 0) {
      ...
      updateData[`system.attributes.hp.value`] = Math.min(
        this.system.attributes.hp.value +
          this.system.details.secondWindValue +
          (r.total || 0),
        this.system.attributes.hp.max,
      );
    }
    ...
    if (this.system.details.surges.value > 0) {
      updateData[`system.details.surges.value`] =
        this.system.details.surges.value - 1;
    }
  ...
  async applySecondWindEffect() {
    ...
    const secondwindEffect = new CONFIG.ActiveEffect.documentClass(
      this.system.details.secondwindEffect,
    );
  ...
  async actionPoint(event, options) {
    ...
    if (this.system.actionpoints.value >= 1) {
      ...
      const updateData = {};
      updateData[`system.actionpoints.value`] =
        this.system.actionpoints.value - 1;
  ...
  async createOwnedItem(itemData, options) {
    console.warn(
      "You are referencing Actor4E#createOwnedItem which is deprecated in favor of Item.create or Actor#createEmbeddedDocuments.  This method exists to aid transition compatibility",
    );
    ...
    if (embeddedName === "Item") {
      if (!this.isPC) {
        data.forEach((datum) => {
          ...
          if (["weapon", "equipment"].includes(t))
            initial["system.equipped"] = true;
    ...
    if (limitedUses && consumeUse) {
      ...
      if (uses <= 0)
        ui.notifications.warn(
          game.i18n.format("DND4E.ItemNoUses", { name: item.name }),
        );

      await item.update({
        "system.uses.value": Math.max(
          parseInt(item.system.uses.value || 0) - 1,
          0,
        ),
      });
  ...
  _computeEncumbrance(actorData) {
    ...
    const max = eval(
      Helper.replaceData(actorData.encumbrance.formulaNorm, actorData)
        .toString()
        .replace(/[^-()\d/*+. ]/g, ""),
    );
    const maxHeavy = eval(
      Helper.replaceData(actorData.encumbrance.formulaHeavy, actorData)
        .toString()
        .replace(/[^-()\d/*+. ]/g, ""),
    );
    const maxMax = eval(
      Helper.replaceData(actorData.encumbrance.formulaMax, actorData)
        .toString()
        .replace(/[^-()\d/*+. ]/g, ""),
    );
  ...
  async calcDamageErrata(damage, multiplier, surges) {
    ...
    for (let d of damage) {
      ...
      if (damageTypesArray.includes("ongoing")) {
        if (actorRes["ongoing"].immune)
          isDamageImmune = actorRes["ongoing"].immune;
      ...
      for (let dt of damageTypesArray) {
        ...
        const currentRes = Helper.sumExtremes([
          resAll,
          actorRes[type]?.value || 0,
        ]);

        if (currentRes !== 0) {
          //if has resistances or vulnerability
          isImmuneAll = false;
          ...
          if (!isDamageImmune) {
            isImmuneAll = false;
            if (resAll) {
              //"Resist all" will stand in for any other damage that does not have a value
              if (resAll < lowestRes) {
                lowestRes = resAll || 0;
              }
    ...
    for (let d of damage) {
      ...
      for (let dt of damageTypesArray) {
        if (damageDealt[dt] === undefined) {
          damageDealt[dt] =
            0 |
            (d[0] / damageTypesArray.length +
              (i < d[0] % damageTypesArray.length));
        } else {
          damageDealt[dt] +=
            0 |
            (d[0] / damageTypesArray.length +
      ...
      if (!isDamageImmune) {
        for (const d in damageDealt) {
          ...
          let res = Helper.sumExtremes([
            resAll,
            actorRes[damagetype]?.value || 0,
          ]);
  ...
  async applyDamage(amount = 0, multiplier = 1, surges = {}) {
    ...
    if (surges.surgeAmount) {
      if (this.system.details.surges.value < surges.surgeAmount) {
        //check to see if enough surges left to use tihs source
        ...
        //check to see if the character is already at max HP
    ...
    if (surges.surgeValueAmount) {
      amount +=
        this.system.details.surgeValue * surges.surgeValueAmount * multiplier;
    }
    ...
    if (amount > 0) {
      ...
      newHp = Math.clamp(
        hp.value - amount,
        -1 * this.system.details.bloodied,
        hp.max,
      );
    } else if (amount < 0) {
      // Healing
    ...
    if (multiplier < 0 && surges.surgeAmount) {
      updates["system.details.surges.value"] =
        this.system.details.surges.value - surges.surgeAmount;
    }
    ...
    return this.createEmbeddedDocuments("ActiveEffect", [
      {
        ...
        duration: {
          rounds: effectData.rounds,
          startRound: effectData.startRound,
        },
  ...
  async promptEoTSavesSocket() {
    ...
    if (toSave.length) {
      ...
      for (let i of toSave) {
        if (isFF) {
          let save = await this.rollSave(event, {
            effectSave: true,
            effectId: i,
          });
        } else {
          let save = await new SaveThrowDialog(this, {
            effectSave: true,
            effectId: i,
          }).render(true);
        }
      ...
      for (const e of this.getActiveEffects()) {
        if (e.flags.dnd4e?.dots.length && e.disabled === false) {
          for (let dot of e.flags.dnd4e.dots) {
            ...
            const types = dot.typesArray.includes("healing")
              ? "healing"
              : dot.typesArray.join(",");
            ...
            try {
              if (!parsedAmount.match(stringDiceFormat))
                parsedAmount = Roll.replaceFormulaData(
                  game.helper.commonReplace(parsedAmount, this),
                  this.getRollData(),
                );
        ...
        for (const dot of applicableDoTs) {
          const dmgTaken =
            dot.type == "healing"
              ...
              : await this.calcDamageInner([[dot.amount, dot.type]]);
          ...
          const chatData = {
            ...
            dmgTaken: dmgTaken,
            dmgDiff:
              Math.max(dot.amount, dmgTaken) - Math.min(dot.amount, dmgTaken),
            typesFormatted: dot.type
              .replaceAll(/,*ongoing,*/g, "")
              .replaceAll(",", " and "),
          ...
          const html = await renderTemplate(
            "systems/dnd4e/templates/chat/ongoing-damage.html",
            chatData,
          );
          ...
          await ChatMessage.create({
            ...
            speaker: {
              actor: this,
              alias: this.isToken ? this.token.name : this.name,
            },
            content: html,
