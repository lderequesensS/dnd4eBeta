...
export default class ActorSheet4e extends ActorSheet {
  ...
  async getData(options) {
    ...
    if (
      Object.entries(game.dnd4e.config.coreSkills).length !=
      Object.entries(actorData.skills).length
    ) {
      const skillNames = Object.keys(actorData.skills);

      // Sort the skill names based on the label property
      skillNames.sort((a, b) =>
        actorData.skills[a].label?.localeCompare(actorData.skills[b].label),
      );
    ...
    this._prepareDataTraits(actorData.languages, {
      spoken: CONFIG.DND4E.spoken,
      script: CONFIG.DND4E.script,
    });
    ...
    this._prepareDataSense(actorData.senses, {
      vision: CONFIG.DND4E.vision,
      special: CONFIG.DND4E.special,
    });

    this._prepareDataSave(actorData.details, { saves: CONFIG.DND4E.saves });
    ...
    data.effects = ActiveEffect4e.prepareActiveEffectCategories(
      actor.getActiveEffects(),
    );

    // Resources
    actorData.resources = ["primary", "secondary", "tertiary"].reduce(
      (obj, r) => {
  ...
  _prepareItems(data) {
    ...
    let [items, pow, feats] = data.items.reduce(
      (arr, item) => {
        // Item details
        item.img = item.img || DEFAULT_TOKEN;
        item.isStack =
          Number.isNumeric(item.system.quantity) && item.system.quantity !== 1;
...
<br>${parseInt(data.system.movement.shift.value)} ${localize("MovementUnit")} ${localize("MovementSpeedShifting")}`;
    ...
    if (data.system.movement.custom) {
      const moveCustom = [];
      data.system.movement.custom
        .split(";")
        .forEach((c, i) => (c ? (moveCustom[i] = c.trim()) : null));
  ...
  _compareValues(key, order = "asc") {
    return function innerSort(a, b) {
      if (a.hasOwnProperty(key) && b.hasOwnProperty(key)) {
        ...
        const varA =
          typeof a.system[key] === "string"
            ? a.system[key].toUpperCase()
            : a.system[key];
        const varB =
          typeof b.system[key] === "string"
            ? b.system[key].toUpperCase()
            : b.system[key];
  ...
  _onSortItem(event, itemData) {
    ...
    const sortUpdates = SortingHelpers.performIntegerSort(source, {
      target: target,
      siblings,
      sortBefore: source.sort > target.sort,
    }); //Changed from const sortUpdates = SortingHelpers.performIntegerSort(source, {target: target, siblings});
  ...
  _groupPowers(power, powerGroups) {
    if (
      this.object.system.powerGroupTypes === "action" ||
      !this.object.system.powerGroupTypes
    ) {
      if (Object.keys(powerGroups).includes(power.system.actionType))
      ...
      if (Object.keys(powerGroups).includes(power.system.powerType))
        return power.system.powerType;
    } else if (this.object.system.powerGroupTypes === "powerSubtype") {
      if (Object.keys(powerGroups).includes(power.system.powerSubtype))
        return power.system.powerSubtype;
    } else if (this.object.system.powerGroupTypes === "usage") {
      if (Object.keys(powerGroups).includes(power.system.useType))
        return power.system.useType;
    }
    return "other";
  }
  ...
  _generatePowerGroups() {
    ...
    const groups =
      DND4E.powerGroupings[powerGroupTypes] ?? DND4E.powerGroupings["usage"]; // paranoid defensive, but ensure we always return something
  ...
  _checkPowerAvailable(itemData) {
    if (
      (!itemData.system.uses.value && itemData.system.preparedMaxUses) ||
  ...
  _preparePowerRangeText(itemData) {
    ...
    if (itemData.system.area) {
      try {
        let areaForm = game.helper.commonReplace(
  ...
  activateListeners(html) {
    ...
    if (this.options.viewPermission) {
      // With at leaste observation permisions, be able to view item summary
      // Item summaries
      html
    ...
    if (this.actor.isOwner) {
      ...
      html
        .find(".resistances-bonus")
        .click(this._onResistancesBonus.bind(this));

      html.find(".movement-dialog").click(this._onMovementDialog.bind(this));

      html
        .find(".custom-roll-descriptions")
        .click(this._onCustomRolldDescriptions.bind(this));
      ...
      html
        .find(".action-point-extra")
        .click(this._onActionPointExtraDialog.bind(this));
      ...
      html
        .find(".trait-selector-weapon")
        .click(this._onTraitSelectorWeapon.bind(this));
      html
        .find(".trait-selector-senses")
        .click(this._onTraitSelectorSense.bind(this));
      html.find(".list-string-input").click(this._onListStringInput.bind(this));

      //save throw bonus
      html
        .find(`.trait-selector-save`)
        .click(this._onTraitSelectorSaveThrow.bind(this));
      ...
      html
        .find(".effect-control")
        .click((event) =>
          ActiveEffect4e.onManageActiveEffect(event, this.actor),
        );
      ...
      html.find(".item .item-image").hover(
        (event) => this._onItemHoverEntry(event),
        (event) => this._onItemHoverExit(event),
      );
      html
      ...
      html
        .find(".encumbrance-options")
        .click(this._onEncumbranceDialog.bind(this));
    ...
    if (this.isEditable) {
      for (const override of this._getAllActorOverrides([
        "system.details.surges.value",
      ])) {
        html
  ...
  _getAllActorOverrides(excluded = []) {
    ...
    const actorKeys = new Set(
      Object.keys(foundry.utils.flattenObject(this.actor)),
    );
    ...
    for (const key of overrides) {
      for (const bonus of bonusSuffixes) {
        ...
        if (key.includes("system.attributes.hp.")) {
          //Exception for HP as to not block
    ...
    console.log(
      Array.from(overrides.union(candidateKeys).intersection(actorKeys)),
    );
  ...
  _onChangeInputDelta(event) {
    ...
    if (["+"].includes(value[0])) {
      ...
      input.value =
        foundry.utils.getProperty(this.actor, input.name) + delta ||
        foundry.utils.getProperty(this.actor, input.name);
  ...
  async _onItemSummary(event) {
    ...
    if (!itemId) {
      console.log(
        "got an item summary event for something without an item id. Assuming its an effect.",
      );
      return;
    }
  ...
  async _generateItemSummary(item) {
    ...
    if (item.type === "power") {
      ...
      if (item.system.autoGenChatPowerCard && chatData.chatFlavor) {
        descrip = $(
          `<div class="item-description">${chatData.chatFlavor}</div>`,
        );
      } else {
        descrip = $(
      ...
      if (item.system.autoGenChatPowerCard) {
        ...
        let detsText = Helper._preparePowerCardData(
          chatData,
          CONFIG,
          this.actor,
          attackBonus,
  ...
  _onToggleItem(event) {
    ...
    const item = this.actor.items.get(itemId);
    const power = ["power", "atwill", "encounter", "daily", "utility"];
    const attr = power.includes(item.type)
      ? "system.prepared"
      : "system.equipped";
  ...
  _onPowerItemCreate(event) {
    ...
    itemData.system.autoGenChatPowerCard = game.settings.get(
      "dnd4e",
      "powerAutoGenerateLableOption",
    );
  ...
  _onSkillBonus(event) {
    ...
    const options = {
      target: target,
      label: `${this.actor.system.skills[skillName].label} Skill Bonus`,
      skill: true,
    };
  ...
  _onDeathSaveBonus(event) {
    event.preventDefault();
    const options = {
      target: `system.details.deathsavebon`,
      label: "Death Savingthrow Bonus",
    };
    new AttributeBonusDialog(this.actor, options).render(true);
  }
  ...
  _onSurgeBonus(event) {
    event.preventDefault();
    const options = {
      target: `system.details.surgeBon`,
      label: "Healing Surge Bonus",
    };
    new AttributeBonusDialog(this.actor, options).render(true);
  }
  ...
  _onSurgeEnv(event) {
    event.preventDefault();
    const options = {
      target: `system.details.surgeEnv`,
      label: "Healing Surges Environmental Losses",
    };
    new AttributeBonusDialog(this.actor, options).render(true);
  }
  ...
  _onSecondWindBonus(event) {
    event.preventDefault();
    const options = {
      target: `system.details.secondwindbon`,
      label: "Second Wind Bonus",
      secondWind: true,
    };
    new AttributeBonusDialog(this.actor, options).render(true);
  }
  ...
  _onDefencesBonus(event) {
    ...
    const options = {
      target: target,
      label: `${this.actor.system.defences[defName].label} ${localize("DefenceBonus")}`,
      ac: defName === "ac",
    };
    new AttributeBonusDialog(this.actor, options).render(true);
  }
  ...
  _onMovementBonus(event) {
    ...
    const options = {
      target: target,
      label: `${this.actor.system.movement[moveName].label} Movement Bonus`,
    };
    new AttributeBonusDialog(this.actor, options).render(true);
  }
  ...
  _onPassiveBonus(event) {
    ...
    const options = {
      target: target,
      label: `Passive ${this.actor.system.skills[skillName].label} Bonus`,
    };
    new AttributeBonusDialog(this.actor, options).render(true);
  }
  ...
  _onModifiersBonus(event) {
    ...
    const options = {
      target: target,
      label: `${this.actor.system.modifiers[modifierName].label} Bonus`,
    };
    new AttributeBonusDialog(this.actor, options).render(true);
  }
  ...
  _onResistancesBonus(event) {
    ...
    const options = {
      target: target,
      label: `${this.actor.system.resistances[resName].label} Damage Resistances Bonus`,
    };
    new AttributeBonusDialog(this.actor, options).render(true);
  }
  ...
  _onSavingThrowBonus(event) {
    event.preventDefault();
    const options = {
      target: `system.details.saves`,
      label: "Savingthrow Bonus",
    };
    new AttributeBonusDialog(this.actor, options).render(true);
  }
  ...
  _onRollEffectSave(event) {
    ...
    let save = new SaveThrowDialog(this.actor, {
      effectSave: true,
      effectId: effectId,
    }).render(true);
  ...
  async _onItemRecharge(event) {
    ...
    if (item.type === "power") {
      if (
        item.system.rechargeRoll ||
        (!item.system.rechargeRoll && !item.system.rechargeCondition)
      ) {
        ...
        if (r.total >= r.dice[0].options.critical) {
          this.object.updateEmbeddedDocuments("Item", [
            { _id: itemId, "system.uses.value": item.system.preparedMaxUses },
          ]);
        ...
        r.toMessage({
          ...
          messageData: {
            "flags.dnd4e.roll": { type: "other", itemId: this.id },
          },
        });
  ...
  _currencyGoldSumLabel() {
    ...
    return (
      localize("DND4E.GoldWealth") +
      (Math.round(goldSum * 100) / 100)
        .toString()
        .replace(/\B(?=(\d{3})+(?!\d))/g, ",")
    );
  }
  ...
  _onTraitSelectorWeapon(event) {
    ...
    const options = {
      ...
      choices,
      datasetOptions: a.dataset.options,
      config: CONFIG,
    };
    new TraitSelector(this.actor, options).render(true);
  }
  ...
  _onItemContext(element) {
    ...
    if (element.classList.contains("effect")) {
      ...
      Hooks.call(
        "DND4E.getActiveEffectContextOptions",
        effect,
    ...
    return [
      ...
      },
      {
        ...
        callback: () =>
          effect.clone(
            {
              name: game.i18n.format("DOCUMENT.CopyOf", { name: effect.name }),
            },
            { save: true },
          ),
      },
      ...
      },
      {
        name: effect.disabled
          ? "DND4E.ContextMenuActionEnable"
          : "DND4E.ContextMenuActionDisable",
  ...
  _getItemContextOptions(item) {
    ...
    if (
      ...
      item.system.attunement !== CONFIG.DND4E.attunementTypes.NONE
    ) {
      const isAttuned =
        item.system.attunement === CONFIG.DND4E.attunementTypes.ATTUNED;
      options.push({
        name: isAttuned
          ? "DND4E.ContextMenuActionUnattune"
          : "DND4E.ContextMenuActionAttune",
        icon: "<i class='fas fa-sun fa-fw'></i>",
        callback: () =>
          item.update({
            "system.attunement":
              CONFIG.DND4E.attunementTypes[isAttuned ? "REQUIRED" : "ATTUNED"],
          }),
      });
    }
    ...
    if ("equipped" in item.system)
      options.push({
        name: item.system.equipped
          ? "DND4E.ContextMenuActionUnequip"
          : "DND4E.ContextMenuActionEquip",
    ...
    if ("prepared" in item.system)
      options.push({
        name: item.system?.prepared
          ? "DND4E.ContextMenuActionUnprepare"
          : "DND4E.ContextMenuActionPrepare",
        icon: "<i class='fas fa-sun fa-fw'></i>",
        callback: () =>
  ...
  async _onDropItemCreate(itemData) {
    ...
    if (
      ...
      !game.settings.get("dnd4e", "disableAdvancements")
    ) {
      ui.notifications.warn(
        game.i18n.format("DND4E.WarnCantAddMultipleAdvancements"),
      );
    ...
    const containers = new Set(
      items.filter((i) => i.type === "backpack").map((i) => i._id),
    );
