...
export default class ActiveEffect4e extends ActiveEffect {
  ...
  get tooltip() {
    ...
    if (this._source.name)
      html += `<div><label class="source">${game.i18n.localize("DND4E.Source")}: ${this._source.name}</label></div>`;
    if (this.duration.label)
      html += `<div><label class="duration">${game.i18n.localize("DND4E.Duration")}: ${this.duration.label}</label></div>`;
    if (this.description)
  ...
  safeEvalEffectValue(actor, change) {
    ...
    // fields.
    if (change.key.indexOf("@") === 0) change.key = change.key.replace("@", "");
    ...
    if (!change.value.match(stringDiceFormat))
      change.value = Roll.replaceFormulaData(
        game.helper.commonReplace(change.value, actor),
        actor.getRollData(),
      );
    ...
    if (this.parent instanceof Actor) {
      ...
      if (combat?.turn != null && combat.turns && combat.turns[combat.turn]) {
        ...
        updates.flags = {
          dnd4e: {
            effectData: {
              startTurnInit: combat.turns[combat.turn].initiative ?? 0,
            },
          },
        };
      }
  ...
  static onManageActiveEffect(event, owner) {
    ...
    const effects = ["Player Character", "NPC"].includes(owner.type)
      ? owner.getActiveEffects()
      : owner.effects.contents;
    const effect = li.dataset.effectId
      ? effects.find((e) => e._id === li.dataset.effectId)
      : null;
  ...
  _prepareDuration() {
    ...
    if (durationType) {
      ...
      return {
        ...
        // remaining: remaining,
        remaining:
          d.rounds == null && d.turns == null && d.seconds == null
            ? null
            : remaining,
  ...
  _getDurationLabel(rounds, turns) {
    ...
    if (durationType) {
      if (durationType === "endOfTargetTurn")
        return game.i18n.localize("DND4E.DurationEndOfTargetTurnSimp");
      else if (durationType === "startOfTargetTurn")
        return game.i18n.localize("DND4E.DurationStartOfTargetTurnSimp");
