...
export default class ActiveEffectConfig4e extends ActiveEffectConfig {
  ...
  async getData(options) {
    ...
    data.powerParent = ["power", "consumable"].includes(
  ...
  activateListeners(html) {
    ...
    html
      .find(".effect-status-control")
      .click(this._onEffectStatusControl.bind(this));
    ...
    switch (button.dataset.action) {
      ...
      case "copy-all":
        const statusId = button
          .closest(".effect-status")
          .getAttribute("data-status-id");
        return this._copyStatusDetails(statusId, button.dataset.action).then(
          () => this.render(),
        );
      case "add":
  ...
  async _copyStatusDetails(statusId, scope = "copy-all") {
    ...
    try {
      ...
      if (scope == "copy-desc" || scope == "copy-all") {
        effectUpdates.description = game.i18n.localize(
          statuses[statusIndex].description,
        );
      }
      ...
      if (game.modules.get("condition-lab-triggler")?.active) {
        if (scope == "copy-flags" || scope == "copy-all") {
          if (statuses[statusIndex].flags["condition-lab-triggler"]) {
            effectUpdates.conditionLab =
              statuses[statusIndex].flags["condition-lab-triggler"];
          }
        }
      }
  ...
  _getSubmitData(updateData = {}) {
    ...
    data.statuses = Array.from(Object.values(data.statuses || {})).filter(
      (x) => x,
    );
    ...
    data.flags.dnd4e.dots = Array.from(
      Object.values(data.flags.dnd4e.dots || {}),
    );
